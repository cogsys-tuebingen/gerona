...
SteerAngleController::SteerAngleController()
...
void SteerAngleController::setLimits (int lower_lim, int upper_lim)
{
...
void SteerAngleController::reset()
{
    sum_t_e_ = 0;
}


void SteerAngleController::setPosition(int pos)
{
    if (pos!=position_) {
        reset();
        position_ = pos;
...
void SteerAngleController::setPiParameters(double kp, double ki)
{
    Kp_ = kp;
    Ki_ = ki;
...
void SteerAngleController::execute(double delta_t_sec, double is_angle_rad, int& servo_val)
{
...
int SteerAngleController::getStaticServoValue (double delta)
{

    /* Mmmh. Couldn't we use the given config values
     to calculate this? Steer servo zero is configurable as well as
     the degree to servo factor! */
