...
void PatternDriver::setGoal(const motion_control::MotionGoal &goal)
{
    cmd_v_ = goal.v;
...
void PatternDriver::start()
{
}


void PatternDriver::stop()
{
    cmd_v_ = 0;
}

bool PatternDriver::update( Vector3d& slam_pose )
{
...
void PatternDriver::generateWaypoints( double delta_f )
{
    waypoints_.clear();
...
void PatternDriver::generateCurve( const Vector3d &curve_start,
                                 const double r,
...
void PatternDriver::generateLine( const Vector3d &line_start,
                                const double length,
                                const double wp_dist,
                                Vector3d &line_end )
{
...
int PatternDriver::execute(MotionFeedback &fb, MotionResult &result)
{
    // check collision
