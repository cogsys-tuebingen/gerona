...
float VoronoiDiagramGenerator::s_scaling_factor = 20000.0f;
...
bool VoronoiDiagramGenerator::generateVoronoi(std::vector<Point> points, float minX, float maxX, float minY, float maxY, float minDist)
{
  cleanup();
	cleanupEdges();
  int i;
  ...
  // make points unique
  std::sort(points.begin(), points.end(), point_smaller_function);
  unique_it = std::unique(points.begin(), points.end(), point_equal_function);
  points.resize( unique_it - points.begin() );

  for(it = points.begin(); it != points.end(); ++it){
    it->x *= s_scaling_factor;
    it->y *= s_scaling_factor;
  }


  minDistanceBetweenSites = minDist * s_scaling_factor;
  ...
  borderMinX = minX * s_scaling_factor;
  borderMinY = minY * s_scaling_factor;
  borderMaxX = maxX * s_scaling_factor;
  borderMaxY = maxY * s_scaling_factor;
...
void VoronoiDiagramGenerator::pushGraphEdge(float x1, float y1, float x2, float y2, struct Edge *e)
{
	GraphEdge* newEdge = new GraphEdge;
	newEdge->next = allEdges;
  allEdges = newEdge;

  newEdge->sites[0].x = e->reg[0]->coord.x;
  newEdge->sites[0].y = e->reg[0]->coord.y;
  newEdge->sites[1].x = e->reg[1]->coord.x;
  newEdge->sites[1].y = e->reg[1]->coord.y;
  newEdge->x1 = x1;
	newEdge->y1 = y1;
	newEdge->x2 = x2;
  newEdge->y2 = y2;
...
void VoronoiDiagramGenerator::openpl(){}
                                      void VoronoiDiagramGenerator::line(float x1, float y1, float x2, float y2, struct Edge *e)
{	
