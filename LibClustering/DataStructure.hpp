...
struct DataStructure {
    ...
    /**
     * @brief pushVector add another vector to the data structure, merge duplicates
     * @param vector
     */
    void pushVector(VectorT vector) {
    ...
    /**
     * @brief pushVectorIfValid create and add a vector to the data structure, if the value is greater than zero
     * @param value weight of the vector
     * @param index the vectors indexes
     */
    template <class ValueProvider, class IndexProvider>
    inline void pushVectorIfValid(const ValueProvider& value, const IndexProvider& index) {
        VectorValueType val = AccessTraits<int, InputDataType>::value(value);
        if(val > 0) {
            pushVector(AccessTraits<int, InputDataType>:: template readVector<VectorT, IndexProvider, VectorValueType>(index, val));
        }
    }

    /**
     * @brief pushVectorIfValid
     * @note see pushVectorIfValid with two parameters
     * @param value object that provides value and indexes
     */
    template <class ValueIndexProvider>
    inline void pushVectorIfValid(const ValueIndexProvider& x) {
        pushVectorIfValid(x, x);
    }
