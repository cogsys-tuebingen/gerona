...
public:
    ...
    struct State
    {
        ...
        State(const Eigen::Vector3d &robot_pose,
              Path::ConstPtr path,
              std::shared_ptr<ObstacleCloud const> obstacle_cloud,
        ...
        const std::shared_ptr<ObstacleCloud const> obstacle_cloud;
    ...
    struct Result
    {
        Result():
            can_continue(true),
            status(path_msgs::FollowPathResult::RESULT_STATUS_UNKNOWN)
        {}
