...
RobotController_Ackermann_OrthogonalExponential::RobotController_Ackermann_OrthogonalExponential(ros::Publisher &cmd_publisher,
...

{
    ...
    look_at_cmd_sub_ = nh_.subscribe<std_msgs::String>("/look_at/cmd", 10,
                                                       boost::bind(&RobotController_Ackermann_OrthogonalExponential::lookAtCommand, this, _1));
    look_at_sub_ = nh_.subscribe<geometry_msgs::PointStamped>("/look_at", 10,
                                                              boost::bind(&RobotController_Ackermann_OrthogonalExponential::lookAt, this, _1));

    laser_sub_front_ = nh_.subscribe<sensor_msgs::LaserScan>("/scan/front/filtered", 10,
                                                                      boost::bind(&RobotController_Ackermann_OrthogonalExponential::laserFront, this, _1));
    laser_sub_back_ = nh_.subscribe<sensor_msgs::LaserScan>("/scan/back/filtered", 10,
                                                                     boost::bind(&RobotController_Ackermann_OrthogonalExponential::laserBack, this, _1));
...
void RobotController_Ackermann_OrthogonalExponential::publishCommand()
{
    if (!cmd_.isValid()) {
        ROS_FATAL("Invalid Command in RobotController_Ackermann_OrthogonalExponential.");
        return;
    }
...
RobotController::ControlStatus RobotController_Ackermann_OrthogonalExponential::execute()
{
    if(N < 2) {
