...
public:
    ...
    */
    ...
    inline void CalculateAngleDiff3(const PoseEvalResults &prevPER, PoseEvalResults &PER) const
    {
        if (prevPER.validState == PERS_NOTASSIGNED || prevPER.validState == PERS_NOTVISIBLE) PER.deltaAngle = 0;
        else
        {
            ...
            PER.deltaAngle = Utils_Math_Approx::facos(std::max(std::max(nd11,nd12),std::max(nd21,nd22)));
            PER.poseCounter = prevPER.poseCounter+1;
        }
    }
    ...
    inline void CalculateAngleDiff(const PoseEvalResults &prevPER, PoseEvalResults &PER) const
    {
        ...
        else
        {
            ...
            PER.deltaAngle = Utils_Math_Approx::facos(std::min(nd11,nd22));
            PER.poseCounter = prevPER.poseCounter+1;
        }
    }
