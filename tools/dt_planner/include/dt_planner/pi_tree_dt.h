...
class PI_TreeDT : public PlannerTrajDT< TS>
{
public:
    typedef std::shared_ptr<PI_TreeDT<TS> > Ptr;
    static PI_TreeDT<TS>::Ptr Create(){ return std::make_shared< PI_TreeDT<TS>  >() ; }
    ...
    void IterateTree(TrajNodeDT* start)
    {
        ...
        std::vector<TrajNodeDT*> newNodes(numSplits);


        for (int i = 0; i < numSplits;++i)
        {
            TrajNodeDT* newNode = CreateTrajectory(start, tempCmds_[i]);
