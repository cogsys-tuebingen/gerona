...
class PI_AStarDT : public PlannerTrajDT<TS>
{
public:
    typedef std::shared_ptr<PI_AStarDT<TS> > Ptr;
    static PI_AStarDT<TS>::Ptr Create(){ return std::make_shared< PI_AStarDT<TS>  >() ; }
    ...
    void IterateStar(TrajNodeDT* start)
    {
        ...
        for (int tl = 0; tl < config_.plannerConfig_.maxSearchIterations;++tl)
        {
            ...
            TrajNodeDT* curNode = openSet_.top();
            openSet_.pop();
            ...
            for (int i = 0; i < numSplits;++i)
            {
                ...
                TrajNodeDT* newNode = CreateTrajectory(curNode,tempCmds_[i]);
    ...
    void Replan()
    {
        TrajectoryDT *result = GetBLResultTrajectory();
        bool doReplan = false;
        if (result == nullptr)
        {
            doReplan = true;
