...
/** @file   vector_char.hh
    ...
    @brief  Vector specialisation for @c char . */

namespace gslmm 
{
  class permutation;

  /** @brief Vector @c char  specialisation. 
      @ingroup vector 
      @param char   The POD of the specialisation. 
  ...
  */ 
  ...
  public:
    ...
    /** Low level data type  */
    typedef gsl_vector_char data_type;
    /** Low level view type  */
    typedef gsl_vector_char_view view_type;
    /** A constant view */
    typedef _gsl_vector_char_const_view const_view_type;
    /** Trait type */
...
#ifndef GSLMM_vectormatrix_matrix_char
...
namespace gslmm
{
  ...
  inline 
  vector<char >::vector(oper_type m)
    : _vector(0)    
  {
    assert(m.get());
    if (!m.get()) GSL_ERROR_VOID("NULL return passed", GSL_EINVAL);
    _vector = gsl_vector_char_calloc(m->size());
    m->operator()(*this);
    // delete m;
    // m = 0;
  }
  ...
  //__________________________________________________________________
  inline 
  vector<char >::vector(size_type n, bool zero)
    : _vector(0)    
  {
    if (zero) _vector = gsl_vector_char_calloc(n);
    else      _vector = gsl_vector_char_alloc(n);
  }

  //__________________________________________________________________
  inline 
  vector<char >::vector(const vector<value_type>& w, bool really)
    : _vector(0)    
  {
    ...
    if (w.is_owner() || really) {
      _vector = gsl_vector_char_alloc(w.size()); 
      *this   = w;
    }
    else {
      view_type v = gsl_vector_char_subvector(w._vector,0,w.size());
      from_view(v);
    }
  }
  ...
  //__________________________________________________________________
  inline 
  vector<char >::vector(const vector<value_type>& w,
			 iterator  offset,
			 size_type n,
			 iterator  stride)
    : _vector(0)    
  {
    view_type v;
    if (stride <= 1) 
      v = gsl_vector_char_subvector(w._vector, offset, n);
    else 
      v = gsl_vector_char_subvector_with_stride(w._vector, offset, stride, n);
    from_view(v);
  }

  //__________________________________________________________________
  inline 
  vector<char >::vector(element_type* a, size_type n, iterator  stride)
    : _vector(0)    
  {
    view_type v;
    if (stride <= 1)
      v = gsl_vector_char_view_array(a, n);
    else
      v = gsl_vector_char_view_array_with_stride(a,stride,n);
    from_view(v);
  }
  ...
  vector<char >::~vector()
  {
    gsl_vector_char_free(_vector);
  }

  //__________________________________________________________________
  inline void 
  vector<char >::from_view(const view_type& v) 
  {
  ...
  //__________________________________________________________________
  inline void 
  vector<char >::from_view(const_view_type& v) 
  {
    _vector = static_cast<data_type*>(malloc(sizeof(data_type)));
    _vector->size   = v.vector.size;
    _vector->stride = v.vector.stride;
  ...
  //__________________________________________________________________
  inline const vector<char >::reference_type 
  vector<char >::operator[](iterator  i) 
    const
  {
#if 0
    return reference_type(gsl_vector_char_const_ptr(_vector,i));
#else
    return reference_type(*gsl_vector_char_const_ptr(_vector,i));
#endif
  }
  ...
  vector<char >::element(iterator  i) const
  {
#if 0
    return (value_type*)(gsl_vector_char_const_ptr(_vector,i));
#else
    return const_cast<value_type*>(gsl_vector_char_const_ptr(_vector,i));
#endif
  }
  ...
  inline 
  vector<char >::value_type* 
  vector<char >::element(iterator  i)
  {
#if 0
    return (value_type*)(gsl_vector_char_ptr(_vector,i));
#else
    return const_cast<value_type*>(gsl_vector_char_ptr(_vector,i));
#endif
  }

  //__________________________________________________________________
  inline void vector<char >::zero()
  { 
  ...
  //__________________________________________________________________
  inline void 
  vector<char >::set(value_type x)
  {
    gsl_vector_char_set_all(_vector,x);
  } 

  //__________________________________________________________________
  inline void 
  vector<char >::basis(iterator  i)
  {
    gsl_vector_char_set_basis(_vector, i);
  }

  //__________________________________________________________________
  inline vector<char > 
  vector<char >::operator()(iterator  offset, size_type n, iterator  stride)
  {
    return vector<char >(*this, offset, n, stride);
  }
  ...
  //__________________________________________________________________
  inline int vector<char >::swap(vector<value_type>& other)
  {
    return gsl_vector_char_swap(_vector, other._vector);
  }

  //__________________________________________________________________
  inline int 
  vector<char >::swap(iterator  i, iterator  j)
  {
    return gsl_vector_char_swap_elements(_vector, i, j);
  }

  //__________________________________________________________________
  inline int vector<char >::reverse()
  {
    return gsl_vector_char_reverse(_vector);
  }

  //__________________________________________________________________
  inline vector<char >& 
  vector<char >::operator=(const vector<value_type>& w)
  {
    gsl_vector_char_memcpy(_vector, w._vector); 
    return *this;
  }
  //__________________________________________________________________
  inline vector<char >& 
  vector<char >::operator=(oper_type m)
  {
    assert(m.get());
  ...
  //__________________________________________________________________
  inline bool 
  vector<char >::is_null() const
  {
    return gsl_vector_char_isnull(_vector) == 1 ? true : false;
  }

#if 0 
  //__________________________________________________________________
  inline matrix<char > 
  vector<char >::outer_product(const vector<value_type> w) const
  {
    matrix<char > m(size(),w.size());
    for (vector<char >::iterator  i = 0; i < size(); i++)
      for (vector<char >::iterator  j = 0; j < w.size(); j++)
	m[i][j] = (*this)[i] * w[j];
    return m;
  } 
#endif
  ...
  //__________________________________________________________________
  inline vector<char >::value_type 
  vector<char >::inner_product(const vector<value_type>& w)  const
  {
    value_type ret;
    for (vector<char >::iterator  i = 0; 
	 i < size() && i < w.size(); i++)
  ...
  {
    return 
      vector<char>(gsl_vector_char_real(this->_vector));
  }

  //__________________________________________________________________
  inline const vector<char> 
  vector<char >::real() const
  {
    return 
      vector<char>(gsl_vector_char_const_real(this->_vector));
  }
  ...
  {
    return 
      vector<char>(gsl_vector_char_imag(this->_vector));
  }

  //__________________________________________________________________
  inline const vector<char> 
  vector<char >::imag() const 
  {
    return 
      vector<char>(gsl_vector_char_const_imag(this->_vector));
  }

#define _GSLMM_VECTOR_FOREACH(w,f) \
  do { for (vector<char >::iterator i = this->begin(); \
...
#define _GSLMM_VECTOR_FOREACH(x,f) \
  do { for (vector<char >::iterator i = this->begin(); i != this->end(); i++)\
    this->operator[](i) f x; } while(false) 
  
  //__________________________________________________________________
  inline vector<char >&
  vector<char >::operator+=(const value_type x)
  {
...
#undef _GSLMM_VECTOR_FOREACH
  ...
  vector<char >::operator+=(const vector<value_type>& w)
  {
    gsl_vector_char_add(_vector, w._vector); return *this;
  }
  ...
  vector<char >::operator-=(const vector<value_type>& w)
  {
    gsl_vector_char_sub(_vector, w._vector); return *this;
  }
  ...
  vector<char >::operator*=(const vector<value_type>& w)
  {
    gsl_vector_char_mul(_vector, w._vector); return *this;
  }
  ...
  vector<char >::operator/=(const vector<value_type>& w)
  {
    gsl_vector_char_div(_vector, w._vector); return *this;
  }

  //__________________________________________________________________
  inline 
  vector<char >& 
  vector<char >::operator+=(const value_type a)
  {
    gsl_vector_char_add_constant(_vector, a); return *this;
  }
  ...
  vector<char >::operator*=(const value_type a)
  {
    gsl_vector_char_scale(_vector, a); return *this;
  }
  ...
  vector<char >::operator/=(const value_type a)
  {
    gsl_vector_char_scale(_vector, 1 / a); return *this;
  }
  ...
  {
    value_type ret = 0;
    for (vector<char >::iterator  i = 0; i < size() && i < w.size(); i++)
      ret += this->operator[](i) * w[i];
    return ret;
  }
  ...
  inline 
  vector<char >::value_type 
  vector<char >::min() const
  {
    return gsl_vector_char_min(_vector);
  }

  //__________________________________________________________________
  inline 
  vector<char >::value_type 
  vector<char >::max() const
  {
    return gsl_vector_char_max(_vector);
  }

  //__________________________________________________________________
  inline 
  void 
  vector<char >::minmax(value_type& min, value_type& max) const
  {
    return gsl_vector_char_minmax(_vector, &min, &max);
  }

  //__________________________________________________________________
  inline 
  vector<char >::iterator  
  vector<char >::min_index() const
  {
    return gsl_vector_char_min_index(_vector);
  }

  //__________________________________________________________________
  inline 
  vector<char >::iterator  
  vector<char >::max_index() const
  {
    return gsl_vector_char_max_index(_vector);
  }

  //__________________________________________________________________
  inline 
  void 
  vector<char >::minmax_index(iterator& min, iterator& max) const
  {
    return gsl_vector_char_minmax_index(_vector, &min, &max);
  }
#endif
}
...
#if 0
#if 0
# ifndef GSLMM_blas_blas_char
#  include <gslmm/blas/blas_char.hh>
# endif
#endif
#endif
