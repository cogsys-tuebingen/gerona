...
#define GSLMM_vector_unsigned_short
...
/** @file   vector_unsigned_short.hh
    @author Christian Holm
    @date   Mon Sep 16 04:41:27 2002
    @brief  Vector specialisation for @c unsigned short . */

namespace gslmm 
{
  class permutation;

  /** @brief Vector @c unsigned short  specialisation. 
      @ingroup vector 
      @param unsigned short   The POD of the specialisation. 
      @c L  is The short-hand type of the specialisation. 
  */ 
  template<>
  class vector<unsigned short  >
  {
  public:
    /** Size type. */
    ...
    /** Low level data type  */
    typedef gsl_vector_ushort data_type;
    /** Low level view type  */
    typedef gsl_vector_ushort_view view_type;
    /** A constant view */
    typedef _gsl_vector_ushort_const_view const_view_type;
    /** Trait type */
...
#ifndef GSLMM_vectormatrix_matrix_unsigned_short
...
namespace gslmm
{
  ...
  inline 
  vector<unsigned short >::vector(oper_type m)
    : _vector(0)    
  {
    assert(m.get());
    if (!m.get()) GSL_ERROR_VOID("NULL return passed", GSL_EINVAL);
    _vector = gsl_vector_ushort_calloc(m->size());
    m->operator()(*this);
    // delete m;
    // m = 0;
  }
  ...
  //__________________________________________________________________
  inline 
  vector<unsigned short >::vector(size_type n, bool zero)
    : _vector(0)    
  {
    if (zero) _vector = gsl_vector_ushort_calloc(n);
    else      _vector = gsl_vector_ushort_alloc(n);
  }

  //__________________________________________________________________
  inline 
  vector<unsigned short >::vector(const vector<value_type>& w, bool really)
    : _vector(0)    
  {
    ...
    if (w.is_owner() || really) {
      _vector = gsl_vector_ushort_alloc(w.size()); 
      *this   = w;
    }
    else {
      view_type v = gsl_vector_ushort_subvector(w._vector,0,w.size());
      from_view(v);
    }
  }
  ...
  //__________________________________________________________________
  inline 
  vector<unsigned short >::vector(const vector<value_type>& w,
			 iterator  offset,
			 size_type n,
			 iterator  stride)
    : _vector(0)    
  {
    view_type v;
    if (stride <= 1) 
      v = gsl_vector_ushort_subvector(w._vector, offset, n);
    else 
      v = gsl_vector_ushort_subvector_with_stride(w._vector, offset, stride, n);
    from_view(v);
  }

  //__________________________________________________________________
  inline 
  vector<unsigned short >::vector(element_type* a, size_type n, iterator  stride)
    : _vector(0)    
  {
    view_type v;
    if (stride <= 1)
      v = gsl_vector_ushort_view_array(a, n);
    else
      v = gsl_vector_ushort_view_array_with_stride(a,stride,n);
    from_view(v);
  }
  ...
  {
    gsl_vector_ushort_free(_vector);
  }

  //__________________________________________________________________
  inline void 
  vector<unsigned short >::from_view(const view_type& v) 
  {
  ...
  //__________________________________________________________________
  inline void 
  vector<unsigned short >::from_view(const_view_type& v) 
  {
    _vector = static_cast<data_type*>(malloc(sizeof(data_type)));
    _vector->size   = v.vector.size;
    _vector->stride = v.vector.stride;
  ...
  //__________________________________________________________________
  inline const vector<unsigned short >::reference_type 
  vector<unsigned short >::operator[](iterator  i) 
    const
  {
#if 0
    return reference_type(gsl_vector_ushort_const_ptr(_vector,i));
#else
    return reference_type(*gsl_vector_ushort_const_ptr(_vector,i));
#endif
  }
  ...
  //__________________________________________________________________
  inline 
  vector<unsigned short >::reference_type
  vector<unsigned short >::operator[](iterator  i)
  {
#if 0
    return reference_type(gsl_vector_ushort_const_ptr(_vector,i));
  ...
  inline 
  const 
  vector<unsigned short >::value_type* 
  vector<unsigned short >::element(iterator  i) const
  {
#if 0
    return (value_type*)(gsl_vector_ushort_const_ptr(_vector,i));
#else
    return const_cast<value_type*>(gsl_vector_ushort_const_ptr(_vector,i));
#endif
  }
  ...
  inline 
  vector<unsigned short >::value_type* 
  vector<unsigned short >::element(iterator  i)
  {
#if 0
    return (value_type*)(gsl_vector_ushort_ptr(_vector,i));
#else
    return const_cast<value_type*>(gsl_vector_ushort_ptr(_vector,i));
#endif
  }

  //__________________________________________________________________
  inline void vector<unsigned short >::zero()
  { 
    gsl_vector_ushort_set_zero(_vector);
  }

  //__________________________________________________________________
  inline void 
  vector<unsigned short >::set(value_type x)
  {
    gsl_vector_ushort_set_all(_vector,x);
  } 

  //__________________________________________________________________
  inline void 
  vector<unsigned short >::basis(iterator  i)
  {
    gsl_vector_ushort_set_basis(_vector, i);
  }

  //__________________________________________________________________
  inline vector<unsigned short > 
  vector<unsigned short >::operator()(iterator  offset, size_type n, iterator  stride)
  {
    return vector<unsigned short >(*this, offset, n, stride);
  }

  //__________________________________________________________________
  inline const vector<unsigned short > 
  vector<unsigned short >::operator()(iterator  offset, size_type n, iterator  stride) 
    const
  {
    return vector<unsigned short >(*this, offset, n, stride);
  ...
  //__________________________________________________________________
  inline int vector<unsigned short >::swap(vector<value_type>& other)
  {
    return gsl_vector_ushort_swap(_vector, other._vector);
  }

  //__________________________________________________________________
  inline int 
  vector<unsigned short >::swap(iterator  i, iterator  j)
  {
    return gsl_vector_ushort_swap_elements(_vector, i, j);
  }

  //__________________________________________________________________
  inline int vector<unsigned short >::reverse()
  {
    return gsl_vector_ushort_reverse(_vector);
  }

  //__________________________________________________________________
  inline vector<unsigned short >& 
  vector<unsigned short >::operator=(const vector<value_type>& w)
  {
    gsl_vector_ushort_memcpy(_vector, w._vector); 
    return *this;
  }
  //__________________________________________________________________
  inline vector<unsigned short >& 
  vector<unsigned short >::operator=(oper_type m)
  {
    assert(m.get());
  ...
  //__________________________________________________________________
  inline bool 
  vector<unsigned short >::is_null() const
  {
    return gsl_vector_ushort_isnull(_vector) == 1 ? true : false;
  }

#if 0 
  //__________________________________________________________________
  inline matrix<unsigned short > 
  vector<unsigned short >::outer_product(const vector<value_type> w) const
  {
    matrix<unsigned short > m(size(),w.size());
    for (vector<unsigned short >::iterator  i = 0; i < size(); i++)
      for (vector<unsigned short >::iterator  j = 0; j < w.size(); j++)
	m[i][j] = (*this)[i] * w[j];
    return m;
  } 
#endif
  ...
  //__________________________________________________________________
  inline vector<unsigned short >::value_type 
  vector<unsigned short >::inner_product(const vector<value_type>& w)  const
  {
    value_type ret;
    for (vector<unsigned short >::iterator  i = 0; 
	 i < size() && i < w.size(); i++)
  ...
  //__________________________________________________________________
  inline vector<unsigned short >::value_type 
  vector<unsigned short >::length() const 
  {
  ...
  //__________________________________________________________________
  inline vector<unsigned short> 
  vector<unsigned short >::real()
  {
    return 
      vector<unsigned short>(gsl_vector_ushort_real(this->_vector));
  }

  //__________________________________________________________________
  inline const vector<unsigned short> 
  vector<unsigned short >::real() const
  {
    return 
      vector<unsigned short>(gsl_vector_ushort_const_real(this->_vector));
  }

  //__________________________________________________________________
  inline vector<unsigned short> 
  vector<unsigned short >::imag()
  {
    return 
      vector<unsigned short>(gsl_vector_ushort_imag(this->_vector));
  }

  //__________________________________________________________________
  inline const vector<unsigned short> 
  vector<unsigned short >::imag() const 
  {
    return 
      vector<unsigned short>(gsl_vector_ushort_const_imag(this->_vector));
  }

#define _GSLMM_VECTOR_FOREACH(w,f) \
  do { for (vector<unsigned short >::iterator i = this->begin(); \
...
#define _GSLMM_VECTOR_FOREACH(x,f) \
  do { for (vector<unsigned short >::iterator i = this->begin(); i != this->end(); i++)\
    this->operator[](i) f x; } while(false) 
  
  //__________________________________________________________________
  inline vector<unsigned short >&
  vector<unsigned short >::operator+=(const value_type x)
  {
...
#undef _GSLMM_VECTOR_FOREACH
  ...
  vector<unsigned short >::operator+=(const vector<value_type>& w)
  {
    gsl_vector_ushort_add(_vector, w._vector); return *this;
  }
  ...
  vector<unsigned short >::operator-=(const vector<value_type>& w)
  {
    gsl_vector_ushort_sub(_vector, w._vector); return *this;
  }
  ...
  vector<unsigned short >::operator*=(const vector<value_type>& w)
  {
    gsl_vector_ushort_mul(_vector, w._vector); return *this;
  }
  ...
  vector<unsigned short >::operator/=(const vector<value_type>& w)
  {
    gsl_vector_ushort_div(_vector, w._vector); return *this;
  }

  //__________________________________________________________________
  inline 
  vector<unsigned short >& 
  vector<unsigned short >::operator+=(const value_type a)
  {
    gsl_vector_ushort_add_constant(_vector, a); return *this;
  }
  ...
  vector<unsigned short >::operator*=(const value_type a)
  {
    gsl_vector_ushort_scale(_vector, a); return *this;
  }
  ...
  vector<unsigned short >::operator/=(const value_type a)
  {
    gsl_vector_ushort_scale(_vector, 1 / a); return *this;
  }
  ...
  {
    value_type ret = 0;
    for (vector<unsigned short >::iterator  i = 0; i < size() && i < w.size(); i++)
      ret += this->operator[](i) * w[i];
    return ret;
  }
  ...
  inline 
  vector<unsigned short >::value_type 
  vector<unsigned short >::min() const
  {
    return gsl_vector_ushort_min(_vector);
  }

  //__________________________________________________________________
  inline 
  vector<unsigned short >::value_type 
  vector<unsigned short >::max() const
  {
    return gsl_vector_ushort_max(_vector);
  }

  //__________________________________________________________________
  inline 
  void 
  vector<unsigned short >::minmax(value_type& min, value_type& max) const
  {
    return gsl_vector_ushort_minmax(_vector, &min, &max);
  }

  //__________________________________________________________________
  inline 
  vector<unsigned short >::iterator  
  vector<unsigned short >::min_index() const
  {
    return gsl_vector_ushort_min_index(_vector);
  }

  //__________________________________________________________________
  inline 
  vector<unsigned short >::iterator  
  vector<unsigned short >::max_index() const
  {
    return gsl_vector_ushort_max_index(_vector);
  }

  //__________________________________________________________________
  inline 
  void 
  vector<unsigned short >::minmax_index(iterator& min, iterator& max) const
  {
    return gsl_vector_ushort_minmax_index(_vector, &min, &max);
  }
#endif
}
...
#if 0
#if 0
# ifndef GSLMM_blas_blas_unsigned_short
#  include <gslmm/blas/blas_unsigned_short.hh>
# endif
#endif
#endif
