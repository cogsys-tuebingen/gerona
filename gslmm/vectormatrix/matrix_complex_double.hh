...
//          options: @skel@, complex<double> .  
//          Do not edit this file.
//
//  gslmm::matrix_complex_double
...
//  02111-1307 USA 
//
#ifndef GSLMM_matrix_complex_double
#define GSLMM_matrix_complex_double
#line 29 "matrix_skel.hh"
/** @file   matrix_complex_double.hh
    @author Christian Holm
    @date   Mon Mar 10 14:50:12 2003
    @brief  @c complex<double>  specialisation of matrix class */
...
#endif
#ifndef GSLMM_vectormatrix_vector_complex_double
# include <gslmm/vectormatrix/vector_complex_double.hh>
#endif
#if 1
# ifndef GSLMM_complex_double
...
namespace gslmm
{
  /** @brief Matrix @c complex<double>  specialisation.
      @ingroup matrix
   */
  template <>
  ...
  public:
    ...
    matrix<complex<double> > operator()(iterator row, iterator column, 
...
namespace gslmm
{
  ...
  inline matrix<complex<double> >::matrix(oper_type p) 
    : _matrix(0)
  {
    assert(p.get());
    if (!p.get()) GSL_ERROR_VOID("NULL return passed", GSL_EINVAL);
    _matrix = gsl_matrix_complex_calloc(p->row_size(), p->column_size());
    p->operator()(*this);
    // delete p;
    // p = 0;
  }
  ...
  //__________________________________________________________________
  inline matrix<complex<double> >::matrix(size_t rows, size_t columns, bool zero) 
    : _matrix(0)
  {
    if (zero) _matrix = gsl_matrix_complex_calloc(rows, columns);
    else      _matrix = gsl_matrix_complex_alloc(rows, columns);
  }
  //__________________________________________________________________
  inline matrix<complex<double> >::matrix(const matrix<value_type>& m,
				bool trans
#if 1
				, bool conj
#endif
  ...
  {
    if (!trans) {
      _matrix = gsl_matrix_complex_alloc(m.row_size(), m.column_size());
      *this = m;
    }
    else {
      _matrix = gsl_matrix_complex_alloc(m.column_size(), m.row_size());
      gsl_matrix_complex_transpose_memcpy(_matrix,m._matrix);
    }
#if 1
    if (conj) conjugate();
#endif
  ...
  //__________________________________________________________________
  inline 
  matrix<complex<double> >::matrix(const matrix<value_type>& m, 
			 iterator row_offset, iterator column_offset, 
			 size_type rows, size_type columns, 
			 size_type) 
    : _matrix(0)
  {
    view_type v = gsl_matrix_complex_submatrix(m._matrix,
					    row_offset, 
					    column_offset,
					    rows, columns);
    from_view(v);
  ...
  //__________________________________________________________________
  inline matrix<complex<double> >::matrix(element_type* array, 
				size_type rows, size_type columns, 
				size_type tda) 
    : _matrix(0)
  {
    view_type v;
    if (tda <= 0)
      v = gsl_matrix_complex_view_array(array, rows, columns);
    else 
      v = gsl_matrix_complex_view_array_with_tda(array, rows, columns, tda);
    from_view(v);
  }
  //__________________________________________________________________
  inline 
  matrix<complex<double> >::matrix(const vector_type& vec,
			 size_type rows, size_type columns, 
			 size_type tda) 
     : _matrix(0)
 {
    view_type v;
    if (tda <= 0)
      v = gsl_matrix_complex_view_vector(vec._vector, rows, columns);
    else 
      v = gsl_matrix_complex_view_vector_with_tda(vec._vector, rows, 
							columns, tda);
    from_view(v);
  }
  
  //__________________________________________________________________
  inline matrix<complex<double> >::~matrix()
  {
    gsl_matrix_complex_free(_matrix);
  }
  //__________________________________________________________________
  inline void matrix<complex<double> >::from_view(view_type& v)
  {
  ...
  matrix<complex<double> >::operator=(const matrix<value_type>& m)
  {
    gsl_matrix_complex_memcpy(_matrix,m._matrix);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<complex<double> >&
  matrix<complex<double> >::operator=(oper_type p)
  {
    assert(p.get());
  ...
  //__________________________________________________________________
  inline void
  matrix<complex<double> >::swap(const matrix<value_type>& m)
  {
    gsl_matrix_complex_swap(_matrix,m._matrix);
  }
  //__________________________________________________________________
  inline void
  matrix<complex<double> >::swap_rows(iterator i, iterator j)
  {
    gsl_matrix_complex_swap_rows(_matrix,i, j);
  }
  //__________________________________________________________________
  inline void
  matrix<complex<double> >::swap_columns(iterator i, iterator j)
  {
    gsl_matrix_complex_swap_columns(_matrix,i, j);
  }
  //__________________________________________________________________
  inline void
  matrix<complex<double> >::swap_row_column(iterator i, iterator j)
  {
    gsl_matrix_complex_swap_rowcol(_matrix,i, j);
  }
  //__________________________________________________________________
  inline bool
  matrix<complex<double> >::is_owner()  const
  {
    return _matrix->owner == 1;
  }
  //__________________________________________________________________
  inline bool
  matrix<complex<double> >::is_null()  const 
  {
    return gsl_matrix_complex_isnull(_matrix) == 1;
  }
  //__________________________________________________________________
  inline void
  matrix<complex<double> >::zero() 
  {
    gsl_matrix_complex_set_zero(_matrix);
  }
  //__________________________________________________________________
  inline void
  matrix<complex<double> >::set(value_type x) 
  {
    gsl_matrix_complex_set_all(_matrix, x);
  }
  //__________________________________________________________________
  inline void
  ...
  //__________________________________________________________________
  inline void
  matrix<complex<double> >::transpose()
  {
    gsl_matrix_complex_transpose(_matrix);
  }
  //__________________________________________________________________
  inline matrix<complex<double> >
  matrix<complex<double> >::operator()(iterator i, iterator j, size_type n1, 
				   size_type n2) const
  {
    return matrix<complex<double> >(*this, i, j, n1, n2);
  }
  //__________________________________________________________________
  inline 
  const matrix<complex<double> >::reference_type
  matrix<complex<double> >::operator()(iterator i, iterator j) const 
  {
#if 1
    return reference_type(gsl_matrix_complex_ptr(_matrix, i, j));
#else 
    return reference_type(*(gsl_matrix_complex_ptr(_matrix, i, j)));
#endif
  }
  //__________________________________________________________________
  inline 
  matrix<complex<double> >::reference_type
  matrix<complex<double> >::operator()(iterator i, iterator j) 
  {
#if 1
    return reference_type(gsl_matrix_complex_ptr(_matrix, i, j));
#else 
  ...
  //__________________________________________________________________
  inline matrix<complex<double> >::vector_type
  matrix<complex<double> >::operator[](iterator i)
  {
    // Must be implemented here - temporary
    return vector_type(gsl_matrix_complex_row(_matrix,i));
  }
  //__________________________________________________________________
  inline const matrix<complex<double> >::vector_type
  matrix<complex<double> >::operator[](iterator i) const
  {
    // Must be implemented here - temporary
    return vector_type(gsl_matrix_complex_row(_matrix,i));
  }
  //__________________________________________________________________
  inline matrix<complex<double> >::value_type*
  matrix<complex<double> >::element(iterator row, iterator column) 
  {
    // Must be implemented here - temporary
    
    return (value_type*)(gsl_matrix_complex_ptr(_matrix,row, column));
  }
  //__________________________________________________________________
  inline const matrix<complex<double> >::value_type*
  matrix<complex<double> >::element(iterator row, iterator column) const
  {
    // Must be implemented here - temporary
    return (value_type*)(gsl_matrix_complex_const_ptr(_matrix,row, column));
  }
  //__________________________________________________________________
  inline matrix<complex<double> >::vector_type
  matrix<complex<double> >::row(iterator i)
  {
    return vector_type(gsl_matrix_complex_row(_matrix,i));
  }
  //__________________________________________________________________
  inline const matrix<complex<double> >::vector_type
  matrix<complex<double> >::row(iterator i) const
  {
    return vector_type(gsl_matrix_complex_row(_matrix,i));
  }
  //__________________________________________________________________
  inline matrix<complex<double> >::vector_type
  matrix<complex<double> >::column(iterator i)
  {
    return vector_type(gsl_matrix_complex_column(_matrix,i));
  }
  //__________________________________________________________________
  inline const matrix<complex<double> >::vector_type
  matrix<complex<double> >::column(iterator i) const
  {
    return vector_type(gsl_matrix_complex_column(_matrix,i));
  }
  //__________________________________________________________________
  inline matrix<complex<double> >::vector_type
  matrix<complex<double> >::diagonal(int i)
  {
    if (i == 0)     
      return vector_type(gsl_matrix_complex_diagonal(_matrix));
    else if (i < 0) 
      return 
	vector_type(gsl_matrix_complex_subdiagonal(_matrix,-i));
    return 
      vector_type(gsl_matrix_complex_superdiagonal(_matrix,i));
  }
  //__________________________________________________________________
  inline const matrix<complex<double> >::vector_type
  matrix<complex<double> >::diagonal(int i) const
  {
    if (i == 0)     
      return vector_type(gsl_matrix_complex_diagonal(_matrix));
    else if (i < 0) 
  ...
  //__________________________________________________________________
  inline matrix<complex<double> >::vector_type
  matrix<complex<double> >::sub_diagonal(int i)
  {
    return diagonal(-i);
  }
  //__________________________________________________________________
  inline const matrix<complex<double> >::vector_type
  matrix<complex<double> >::sub_diagonal(int i) const
  {
    return diagonal(-i);
  }
  //__________________________________________________________________
  inline matrix<complex<double> >::vector_type
  matrix<complex<double> >::super_diagonal(int i)
  {
    return diagonal(i);
  }
  //__________________________________________________________________
  inline const matrix<complex<double> >::vector_type
  matrix<complex<double> >::super_diagonal(int i) const
  {
    return diagonal(i);
  }
  //__________________________________________________________________
  ...
  matrix<complex<double> >::operator*=(const matrix<value_type>& m)
  {
    gsl_matrix_complex_mul_elements(_matrix,m._matrix);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<complex<double> >&
  matrix<complex<double> >::operator/=(const matrix<value_type>& m)
  {
    gsl_matrix_complex_div_elements(_matrix,m._matrix);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<complex<double> >&
  matrix<complex<double> >::operator+=(const matrix<value_type>& m)
  {
    gsl_matrix_complex_add(_matrix,m._matrix);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<complex<double> >&
  matrix<complex<double> >::operator-=(const matrix<value_type>& m)
  {
    gsl_matrix_complex_sub(_matrix,m._matrix);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<complex<double> >&
  matrix<complex<double> >::operator+=(const value_type& x)
  {
    gsl_matrix_complex_add_constant(_matrix,x);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<complex<double> >&
  ...
  matrix<complex<double> >::operator*=(const value_type& x)
  {
    gsl_matrix_complex_scale(_matrix,x);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<complex<double> >&
  ...
  inline 
  void
  matrix<complex<double> >::conjugate() 
  {
  ...
  //__________________________________________________________________
  inline 
  matrix<complex<double> >::value_type
  matrix<complex<double> >::min() const 
  {
    return gsl_matrix_complex_min(_matrix);
  }
  //__________________________________________________________________
  inline 
  matrix<complex<double> >::value_type
  matrix<complex<double> >::max() const 
  {
    return gsl_matrix_complex_max(_matrix);
  }
  //__________________________________________________________________
  inline void
  matrix<complex<double> >::minmax(value_type& min, value_type& max) const 
  {
    return gsl_matrix_complex_minmax(_matrix, &min, &max);
  }
  //__________________________________________________________________
  inline void
  matrix<complex<double> >::min_index(iterator& row, iterator& column) const 
  {
    gsl_matrix_complex_min_index(_matrix, &row, &column);
  }
  //__________________________________________________________________
  inline void
  matrix<complex<double> >::max_index(iterator& row, iterator& column) const 
  {
    gsl_matrix_complex_max_index(_matrix, &row, &column);
  }
  //__________________________________________________________________
  inline void
  matrix<complex<double> >::minmax_index(iterator& min_row, iterator& min_column,
			       iterator& max_row, iterator& max_column) 
    const 
  {
    gsl_matrix_complex_minmax_index(_matrix, &min_row, &min_column,
					  &max_row, &max_column);
  }
#endif
}
