...
#define GSLMM_vector_complex_double
...
/** @file   vector_complex_double.hh
    @author Christian Holm
    @date   Mon Sep 16 04:41:27 2002
    @brief  Vector specialisation for @c complex<double> . */

namespace gslmm 
{
  class permutation;

  /** @brief Vector @c complex<double>  specialisation. 
      @ingroup vector 
      @param complex<double>   The POD of the specialisation. 
  ...
  */ 
  ...
  public:
    ...
    /** Low level data type  */
    typedef gsl_vector_complex data_type;
    /** Low level view type  */
    typedef gsl_vector_complex_view view_type;
    /** A constant view */
    typedef _gsl_vector_complex_const_view const_view_type;
    /** Trait type */
...
#ifndef GSLMM_vectormatrix_matrix_complex_double
...
namespace gslmm
{
  ...
  inline 
  vector<complex<double> >::vector(oper_type m)
    : _vector(0)    
  {
    assert(m.get());
    if (!m.get()) GSL_ERROR_VOID("NULL return passed", GSL_EINVAL);
    _vector = gsl_vector_complex_calloc(m->size());
    m->operator()(*this);
    // delete m;
    // m = 0;
  }
  ...
  //__________________________________________________________________
  inline 
  vector<complex<double> >::vector(size_type n, bool zero)
    : _vector(0)    
  {
    if (zero) _vector = gsl_vector_complex_calloc(n);
    else      _vector = gsl_vector_complex_alloc(n);
  }

  //__________________________________________________________________
  inline 
  vector<complex<double> >::vector(const vector<value_type>& w, bool really)
    : _vector(0)    
  {
    ...
    if (w.is_owner() || really) {
      _vector = gsl_vector_complex_alloc(w.size()); 
      *this   = w;
    }
    else {
      view_type v = gsl_vector_complex_subvector(w._vector,0,w.size());
      from_view(v);
    }
  }
  ...
  //__________________________________________________________________
  inline 
  vector<complex<double> >::vector(const vector<value_type>& w,
			 iterator  offset,
			 size_type n,
			 iterator  stride)
    : _vector(0)    
  {
    view_type v;
    if (stride <= 1) 
      v = gsl_vector_complex_subvector(w._vector, offset, n);
    else 
      v = gsl_vector_complex_subvector_with_stride(w._vector, offset, stride, n);
    from_view(v);
  }

  //__________________________________________________________________
  inline 
  vector<complex<double> >::vector(element_type* a, size_type n, iterator  stride)
    : _vector(0)    
  {
    view_type v;
    if (stride <= 1)
      v = gsl_vector_complex_view_array(a, n);
    else
      v = gsl_vector_complex_view_array_with_stride(a,stride,n);
    from_view(v);
  }
  ...
  //__________________________________________________________________
  inline void 
  vector<complex<double> >::from_view(const view_type& v) 
  {
  ...
  //__________________________________________________________________
  inline void 
  vector<complex<double> >::from_view(const_view_type& v) 
  {
    _vector = static_cast<data_type*>(malloc(sizeof(data_type)));
    _vector->size   = v.vector.size;
    _vector->stride = v.vector.stride;
  ...
  //__________________________________________________________________
  inline const vector<complex<double> >::reference_type 
  vector<complex<double> >::operator[](iterator  i) 
    const
  {
#if 1
    return reference_type(gsl_vector_complex_const_ptr(_vector,i));
#else
    return reference_type(*gsl_vector_complex_const_ptr(_vector,i));
#endif
  }
  ...
  //__________________________________________________________________
  inline 
  vector<complex<double> >::reference_type
  vector<complex<double> >::operator[](iterator  i)
  {
#if 1
    return reference_type(gsl_vector_complex_const_ptr(_vector,i));
  ...
  inline 
  const 
  vector<complex<double> >::value_type* 
  vector<complex<double> >::element(iterator  i) const
  {
#if 1
    return (value_type*)(gsl_vector_complex_const_ptr(_vector,i));
#else
    return const_cast<value_type*>(gsl_vector_complex_const_ptr(_vector,i));
#endif
  }
  ...
  inline 
  vector<complex<double> >::value_type* 
  vector<complex<double> >::element(iterator  i)
  {
#if 1
    return (value_type*)(gsl_vector_complex_ptr(_vector,i));
#else
    return const_cast<value_type*>(gsl_vector_complex_ptr(_vector,i));
#endif
  }

  //__________________________________________________________________
  inline void vector<complex<double> >::zero()
  { 
  ...
  //__________________________________________________________________
  inline void 
  vector<complex<double> >::set(value_type x)
  {
    gsl_vector_complex_set_all(_vector,x);
  } 

  //__________________________________________________________________
  inline void 
  vector<complex<double> >::basis(iterator  i)
  {
    gsl_vector_complex_set_basis(_vector, i);
  }

  //__________________________________________________________________
  inline vector<complex<double> > 
  vector<complex<double> >::operator()(iterator  offset, size_type n, iterator  stride)
  {
    return vector<complex<double> >(*this, offset, n, stride);
  }

  //__________________________________________________________________
  inline const vector<complex<double> > 
  vector<complex<double> >::operator()(iterator  offset, size_type n, iterator  stride) 
    const
  {
    return vector<complex<double> >(*this, offset, n, stride);
  ...
  //__________________________________________________________________
  inline int vector<complex<double> >::swap(vector<value_type>& other)
  {
    return gsl_vector_complex_swap(_vector, other._vector);
  }

  //__________________________________________________________________
  inline int 
  vector<complex<double> >::swap(iterator  i, iterator  j)
  {
    return gsl_vector_complex_swap_elements(_vector, i, j);
  }

  //__________________________________________________________________
  inline int vector<complex<double> >::reverse()
  {
    return gsl_vector_complex_reverse(_vector);
  }

  //__________________________________________________________________
  inline vector<complex<double> >& 
  vector<complex<double> >::operator=(const vector<value_type>& w)
  {
    gsl_vector_complex_memcpy(_vector, w._vector); 
    return *this;
  }
  //__________________________________________________________________
  inline vector<complex<double> >& 
  vector<complex<double> >::operator=(oper_type m)
  {
    assert(m.get());
  ...
  //__________________________________________________________________
  inline bool 
  vector<complex<double> >::is_null() const
  {
    return gsl_vector_complex_isnull(_vector) == 1 ? true : false;
  }

#if 0 
  //__________________________________________________________________
  inline matrix<complex<double> > 
  vector<complex<double> >::outer_product(const vector<value_type> w) const
  {
    matrix<complex<double> > m(size(),w.size());
    for (vector<complex<double> >::iterator  i = 0; i < size(); i++)
      for (vector<complex<double> >::iterator  j = 0; j < w.size(); j++)
	m[i][j] = (*this)[i] * w[j];
    return m;
  } 
#endif
  ...
  //__________________________________________________________________
  inline vector<complex<double> >::value_type 
  vector<complex<double> >::inner_product(const vector<value_type>& w)  const
  {
    value_type ret;
    for (vector<complex<double> >::iterator  i = 0; 
	 i < size() && i < w.size(); i++)
  ...
  //__________________________________________________________________
  inline vector<complex<double> >::value_type 
  vector<complex<double> >::length() const 
  {
  ...
  vector<complex<double> >::real()
  {
    return 
      vector<double>(gsl_vector_complex_real(this->_vector));
  }

  //__________________________________________________________________
  inline const vector<double> 
  vector<complex<double> >::real() const
  {
    return 
      vector<double>(gsl_vector_complex_const_real(this->_vector));
  }

  //__________________________________________________________________
  inline vector<double> 
  vector<complex<double> >::imag()
  {
    return 
      vector<double>(gsl_vector_complex_imag(this->_vector));
  }

  //__________________________________________________________________
  inline const vector<double> 
  vector<complex<double> >::imag() const 
  {
    return 
      vector<double>(gsl_vector_complex_const_imag(this->_vector));
  }
...
#define _GSLMM_VECTOR_FOREACH(x,f) \
  ...
  //__________________________________________________________________
  inline vector<complex<double> >&
  vector<complex<double> >::operator+=(const value_type x)
  {
...
#undef _GSLMM_VECTOR_FOREACH
  ...
  vector<complex<double> >::operator+=(const vector<value_type>& w)
  {
    gsl_vector_complex_add(_vector, w._vector); return *this;
  }
  ...
  vector<complex<double> >::operator-=(const vector<value_type>& w)
  {
    gsl_vector_complex_sub(_vector, w._vector); return *this;
  }
  ...
  vector<complex<double> >::operator*=(const vector<value_type>& w)
  {
    gsl_vector_complex_mul(_vector, w._vector); return *this;
  }
  ...
  vector<complex<double> >::operator/=(const vector<value_type>& w)
  {
    gsl_vector_complex_div(_vector, w._vector); return *this;
  }

  //__________________________________________________________________
  inline 
  vector<complex<double> >& 
  vector<complex<double> >::operator+=(const value_type a)
  {
    gsl_vector_complex_add_constant(_vector, a); return *this;
  }
  ...
  vector<complex<double> >::operator*=(const value_type a)
  {
    gsl_vector_complex_scale(_vector, a); return *this;
  }
  ...
  vector<complex<double> >::operator/=(const value_type a)
  {
    gsl_vector_complex_scale(_vector, 1 / a); return *this;
  }
  ...
  {
    value_type ret = 0;
    for (vector<complex<double> >::iterator  i = 0; i < size() && i < w.size(); i++)
      ret += this->operator[](i) * w[i];
    return ret;
  }
  ...
  inline 
  vector<complex<double> >::value_type 
  vector<complex<double> >::min() const
  {
    return gsl_vector_complex_min(_vector);
  }

  //__________________________________________________________________
  inline 
  vector<complex<double> >::value_type 
  vector<complex<double> >::max() const
  {
    return gsl_vector_complex_max(_vector);
  }

  //__________________________________________________________________
  inline 
  void 
  vector<complex<double> >::minmax(value_type& min, value_type& max) const
  {
    return gsl_vector_complex_minmax(_vector, &min, &max);
  }

  //__________________________________________________________________
  inline 
  vector<complex<double> >::iterator  
  vector<complex<double> >::min_index() const
  {
    return gsl_vector_complex_min_index(_vector);
  }

  //__________________________________________________________________
  inline 
  vector<complex<double> >::iterator  
  vector<complex<double> >::max_index() const
  {
    return gsl_vector_complex_max_index(_vector);
  }

  //__________________________________________________________________
  inline 
  void 
  vector<complex<double> >::minmax_index(iterator& min, iterator& max) const
  {
    return gsl_vector_complex_minmax_index(_vector, &min, &max);
  }
#endif
}
...
#if 0
#if 1
# ifndef GSLMM_blas_blas_complex_double
