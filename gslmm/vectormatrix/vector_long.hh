...
/** @file   vector_long.hh
    ...
    @brief  Vector specialisation for @c long . */

namespace gslmm 
{
  class permutation;

  /** @brief Vector @c long  specialisation. 
      @ingroup vector 
      @param long   The POD of the specialisation. 
  ...
  */ 
  ...
  public:
    ...
    /** Low level data type  */
    typedef gsl_vector_long data_type;
    /** Low level view type  */
    typedef gsl_vector_long_view view_type;
    /** A constant view */
    typedef _gsl_vector_long_const_view const_view_type;
    /** Trait type */
...
#ifndef GSLMM_vectormatrix_matrix_long
...
namespace gslmm
{
  ...
  inline 
  vector<long >::vector(oper_type m)
    : _vector(0)    
  {
    assert(m.get());
    if (!m.get()) GSL_ERROR_VOID("NULL return passed", GSL_EINVAL);
    _vector = gsl_vector_long_calloc(m->size());
    m->operator()(*this);
    // delete m;
    // m = 0;
  }
  ...
  //__________________________________________________________________
  inline 
  vector<long >::vector(size_type n, bool zero)
    : _vector(0)    
  {
    if (zero) _vector = gsl_vector_long_calloc(n);
    else      _vector = gsl_vector_long_alloc(n);
  }

  //__________________________________________________________________
  inline 
  vector<long >::vector(const vector<value_type>& w, bool really)
    : _vector(0)    
  {
    ...
    if (w.is_owner() || really) {
      _vector = gsl_vector_long_alloc(w.size()); 
      *this   = w;
    }
    else {
      view_type v = gsl_vector_long_subvector(w._vector,0,w.size());
      from_view(v);
    }
  }
  ...
  //__________________________________________________________________
  inline 
  vector<long >::vector(const vector<value_type>& w,
			 iterator  offset,
			 size_type n,
			 iterator  stride)
    : _vector(0)    
  {
    view_type v;
    if (stride <= 1) 
      v = gsl_vector_long_subvector(w._vector, offset, n);
    else 
      v = gsl_vector_long_subvector_with_stride(w._vector, offset, stride, n);
    from_view(v);
  }

  //__________________________________________________________________
  inline 
  vector<long >::vector(element_type* a, size_type n, iterator  stride)
    : _vector(0)    
  {
    view_type v;
    if (stride <= 1)
      v = gsl_vector_long_view_array(a, n);
    else
      v = gsl_vector_long_view_array_with_stride(a,stride,n);
    from_view(v);
  }
  ...
  vector<long >::~vector()
  {
    gsl_vector_long_free(_vector);
  }

  //__________________________________________________________________
  inline void 
  vector<long >::from_view(const view_type& v) 
  {
  ...
  //__________________________________________________________________
  inline void 
  vector<long >::from_view(const_view_type& v) 
  {
    _vector = static_cast<data_type*>(malloc(sizeof(data_type)));
    _vector->size   = v.vector.size;
    _vector->stride = v.vector.stride;
  ...
  //__________________________________________________________________
  inline const vector<long >::reference_type 
  vector<long >::operator[](iterator  i) 
    const
  {
#if 0
    return reference_type(gsl_vector_long_const_ptr(_vector,i));
#else
    return reference_type(*gsl_vector_long_const_ptr(_vector,i));
#endif
  }
  ...
  vector<long >::element(iterator  i) const
  {
#if 0
    return (value_type*)(gsl_vector_long_const_ptr(_vector,i));
#else
    return const_cast<value_type*>(gsl_vector_long_const_ptr(_vector,i));
#endif
  }
  ...
  inline 
  vector<long >::value_type* 
  vector<long >::element(iterator  i)
  {
#if 0
    return (value_type*)(gsl_vector_long_ptr(_vector,i));
#else
    return const_cast<value_type*>(gsl_vector_long_ptr(_vector,i));
#endif
  }

  //__________________________________________________________________
  inline void vector<long >::zero()
  { 
    gsl_vector_long_set_zero(_vector);
  }

  //__________________________________________________________________
  inline void 
  vector<long >::set(value_type x)
  {
    gsl_vector_long_set_all(_vector,x);
  } 

  //__________________________________________________________________
  inline void 
  vector<long >::basis(iterator  i)
  {
    gsl_vector_long_set_basis(_vector, i);
  }

  //__________________________________________________________________
  inline vector<long > 
  vector<long >::operator()(iterator  offset, size_type n, iterator  stride)
  {
    return vector<long >(*this, offset, n, stride);
  }

  //__________________________________________________________________
  inline const vector<long > 
  vector<long >::operator()(iterator  offset, size_type n, iterator  stride) 
    const
  {
    return vector<long >(*this, offset, n, stride);
  ...
  //__________________________________________________________________
  inline int vector<long >::swap(vector<value_type>& other)
  {
    return gsl_vector_long_swap(_vector, other._vector);
  }

  //__________________________________________________________________
  inline int 
  vector<long >::swap(iterator  i, iterator  j)
  {
    return gsl_vector_long_swap_elements(_vector, i, j);
  }

  //__________________________________________________________________
  inline int vector<long >::reverse()
  {
    return gsl_vector_long_reverse(_vector);
  }

  //__________________________________________________________________
  inline vector<long >& 
  vector<long >::operator=(const vector<value_type>& w)
  {
    gsl_vector_long_memcpy(_vector, w._vector); 
    return *this;
  }
  //__________________________________________________________________
  inline vector<long >& 
  vector<long >::operator=(oper_type m)
  {
    assert(m.get());
  ...
  //__________________________________________________________________
  inline bool 
  vector<long >::is_null() const
  {
    return gsl_vector_long_isnull(_vector) == 1 ? true : false;
  }

#if 0 
  //__________________________________________________________________
  inline matrix<long > 
  vector<long >::outer_product(const vector<value_type> w) const
  {
    matrix<long > m(size(),w.size());
    for (vector<long >::iterator  i = 0; i < size(); i++)
      for (vector<long >::iterator  j = 0; j < w.size(); j++)
	m[i][j] = (*this)[i] * w[j];
    return m;
  } 
#endif
  ...
  //__________________________________________________________________
  inline vector<long >::value_type 
  vector<long >::inner_product(const vector<value_type>& w)  const
  {
    value_type ret;
    for (vector<long >::iterator  i = 0; 
	 i < size() && i < w.size(); i++)
  ...
  {
    return 
      vector<long>(gsl_vector_long_real(this->_vector));
  }

  //__________________________________________________________________
  inline const vector<long> 
  vector<long >::real() const
  {
    return 
      vector<long>(gsl_vector_long_const_real(this->_vector));
  }
  ...
  {
    return 
      vector<long>(gsl_vector_long_imag(this->_vector));
  }

  //__________________________________________________________________
  inline const vector<long> 
  vector<long >::imag() const 
  {
    return 
      vector<long>(gsl_vector_long_const_imag(this->_vector));
  }

#define _GSLMM_VECTOR_FOREACH(w,f) \
  do { for (vector<long >::iterator i = this->begin(); \
...
#define _GSLMM_VECTOR_FOREACH(x,f) \
  do { for (vector<long >::iterator i = this->begin(); i != this->end(); i++)\
    this->operator[](i) f x; } while(false) 
  
  //__________________________________________________________________
  inline vector<long >&
  vector<long >::operator+=(const value_type x)
  {
...
#undef _GSLMM_VECTOR_FOREACH
  ...
  vector<long >::operator+=(const vector<value_type>& w)
  {
    gsl_vector_long_add(_vector, w._vector); return *this;
  }
  ...
  vector<long >::operator-=(const vector<value_type>& w)
  {
    gsl_vector_long_sub(_vector, w._vector); return *this;
  }
  ...
  vector<long >::operator*=(const vector<value_type>& w)
  {
    gsl_vector_long_mul(_vector, w._vector); return *this;
  }
  ...
  vector<long >::operator/=(const vector<value_type>& w)
  {
    gsl_vector_long_div(_vector, w._vector); return *this;
  }

  //__________________________________________________________________
  inline 
  vector<long >& 
  vector<long >::operator+=(const value_type a)
  {
    gsl_vector_long_add_constant(_vector, a); return *this;
  }
  ...
  vector<long >::operator*=(const value_type a)
  {
    gsl_vector_long_scale(_vector, a); return *this;
  }
  ...
  vector<long >::operator/=(const value_type a)
  {
    gsl_vector_long_scale(_vector, 1 / a); return *this;
  }
  ...
  {
    value_type ret = 0;
    for (vector<long >::iterator  i = 0; i < size() && i < w.size(); i++)
      ret += this->operator[](i) * w[i];
    return ret;
  }
  ...
  inline 
  vector<long >::value_type 
  vector<long >::min() const
  {
    return gsl_vector_long_min(_vector);
  }

  //__________________________________________________________________
  inline 
  vector<long >::value_type 
  vector<long >::max() const
  {
    return gsl_vector_long_max(_vector);
  }

  //__________________________________________________________________
  inline 
  void 
  vector<long >::minmax(value_type& min, value_type& max) const
  {
    return gsl_vector_long_minmax(_vector, &min, &max);
  }

  //__________________________________________________________________
  inline 
  vector<long >::iterator  
  vector<long >::min_index() const
  {
    return gsl_vector_long_min_index(_vector);
  }

  //__________________________________________________________________
  inline 
  vector<long >::iterator  
  vector<long >::max_index() const
  {
    return gsl_vector_long_max_index(_vector);
  }

  //__________________________________________________________________
  inline 
  void 
  vector<long >::minmax_index(iterator& min, iterator& max) const
  {
    return gsl_vector_long_minmax_index(_vector, &min, &max);
  }
#endif
}
...
#if 0
#if 0
# ifndef GSLMM_blas_blas_long
#  include <gslmm/blas/blas_long.hh>
# endif
#endif
#endif
