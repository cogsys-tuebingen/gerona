...
#define GSLMM_matrix_unsigned_char
#line 29 "matrix_skel.hh"
/** @file   matrix_unsigned_char.hh
    @author Christian Holm
    @date   Mon Mar 10 14:50:12 2003
    @brief  @c unsigned char  specialisation of matrix class */
...
#endif
#ifndef GSLMM_vectormatrix_vector_unsigned_char
...
namespace gslmm
{
  /** @brief Matrix @c unsigned char  specialisation.
      @ingroup matrix
   */
  template <>
  ...
  public:
    ...
    /** Low level data type */
    typedef gsl_matrix_uchar data_type;
    /** Low level view type */
    typedef gsl_matrix_uchar_view view_type;
    ...
    matrix<unsigned char > operator()(iterator row, iterator column, 
...
namespace gslmm
{
  ...
  inline matrix<unsigned char >::matrix(oper_type p) 
    : _matrix(0)
  {
    assert(p.get());
    if (!p.get()) GSL_ERROR_VOID("NULL return passed", GSL_EINVAL);
    _matrix = gsl_matrix_uchar_calloc(p->row_size(), p->column_size());
    p->operator()(*this);
    // delete p;
    // p = 0;
  }
  ...
  //__________________________________________________________________
  inline matrix<unsigned char >::matrix(size_t rows, size_t columns, bool zero) 
    : _matrix(0)
  {
    if (zero) _matrix = gsl_matrix_uchar_calloc(rows, columns);
    else      _matrix = gsl_matrix_uchar_alloc(rows, columns);
  }
  //__________________________________________________________________
  inline matrix<unsigned char >::matrix(const matrix<value_type>& m,
				bool trans
#if 0
				, bool conj
#endif
  ...
  {
    if (!trans) {
      _matrix = gsl_matrix_uchar_alloc(m.row_size(), m.column_size());
      *this = m;
    }
    else {
      _matrix = gsl_matrix_uchar_alloc(m.column_size(), m.row_size());
      gsl_matrix_uchar_transpose_memcpy(_matrix,m._matrix);
    }
#if 0
    if (conj) conjugate();
#endif
  ...
  //__________________________________________________________________
  inline 
  matrix<unsigned char >::matrix(const matrix<value_type>& m, 
			 iterator row_offset, iterator column_offset, 
			 size_type rows, size_type columns, 
			 size_type) 
    : _matrix(0)
  {
    view_type v = gsl_matrix_uchar_submatrix(m._matrix,
					    row_offset, 
					    column_offset,
					    rows, columns);
    from_view(v);
  ...
  //__________________________________________________________________
  inline matrix<unsigned char >::matrix(element_type* array, 
				size_type rows, size_type columns, 
				size_type tda) 
    : _matrix(0)
  {
    view_type v;
    if (tda <= 0)
      v = gsl_matrix_uchar_view_array(array, rows, columns);
    else 
      v = gsl_matrix_uchar_view_array_with_tda(array, rows, columns, tda);
    from_view(v);
  }
  //__________________________________________________________________
  inline 
  matrix<unsigned char >::matrix(const vector_type& vec,
			 size_type rows, size_type columns, 
			 size_type tda) 
     : _matrix(0)
 {
    view_type v;
    if (tda <= 0)
      v = gsl_matrix_uchar_view_vector(vec._vector, rows, columns);
    else 
      v = gsl_matrix_uchar_view_vector_with_tda(vec._vector, rows, 
							columns, tda);
    from_view(v);
  }
  
  //__________________________________________________________________
  inline matrix<unsigned char >::~matrix()
  {
    gsl_matrix_uchar_free(_matrix);
  }
  //__________________________________________________________________
  inline void matrix<unsigned char >::from_view(view_type& v)
  {
  ...
  matrix<unsigned char >::operator=(const matrix<value_type>& m)
  {
    gsl_matrix_uchar_memcpy(_matrix,m._matrix);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<unsigned char >&
  matrix<unsigned char >::operator=(oper_type p)
  {
    assert(p.get());
  ...
  //__________________________________________________________________
  inline void
  matrix<unsigned char >::swap(const matrix<value_type>& m)
  {
    gsl_matrix_uchar_swap(_matrix,m._matrix);
  }
  //__________________________________________________________________
  inline void
  matrix<unsigned char >::swap_rows(iterator i, iterator j)
  {
    gsl_matrix_uchar_swap_rows(_matrix,i, j);
  }
  //__________________________________________________________________
  inline void
  matrix<unsigned char >::swap_columns(iterator i, iterator j)
  {
    gsl_matrix_uchar_swap_columns(_matrix,i, j);
  }
  //__________________________________________________________________
  inline void
  matrix<unsigned char >::swap_row_column(iterator i, iterator j)
  {
    gsl_matrix_uchar_swap_rowcol(_matrix,i, j);
  }
  //__________________________________________________________________
  inline bool
  matrix<unsigned char >::is_owner()  const
  {
    return _matrix->owner == 1;
  }
  //__________________________________________________________________
  inline bool
  matrix<unsigned char >::is_null()  const 
  {
    return gsl_matrix_uchar_isnull(_matrix) == 1;
  }
  //__________________________________________________________________
  inline void
  matrix<unsigned char >::zero() 
  {
    gsl_matrix_uchar_set_zero(_matrix);
  }
  //__________________________________________________________________
  inline void
  matrix<unsigned char >::set(value_type x) 
  {
    gsl_matrix_uchar_set_all(_matrix, x);
  }
  //__________________________________________________________________
  inline void
  matrix<unsigned char >::identity()
  {
    gsl_matrix_uchar_set_identity(_matrix);
  }
  //__________________________________________________________________
  inline void
  matrix<unsigned char >::transpose()
  {
    gsl_matrix_uchar_transpose(_matrix);
  }
  //__________________________________________________________________
  inline matrix<unsigned char >
  matrix<unsigned char >::operator()(iterator i, iterator j, size_type n1, 
				   size_type n2) const
  {
    return matrix<unsigned char >(*this, i, j, n1, n2);
  }
  //__________________________________________________________________
  inline 
  const matrix<unsigned char >::reference_type
  matrix<unsigned char >::operator()(iterator i, iterator j) const 
  {
#if 0
    return reference_type(gsl_matrix_uchar_ptr(_matrix, i, j));
#else 
    return reference_type(*(gsl_matrix_uchar_ptr(_matrix, i, j)));
#endif
  }
  //__________________________________________________________________
  inline 
  matrix<unsigned char >::reference_type
  matrix<unsigned char >::operator()(iterator i, iterator j) 
  {
#if 0
    return reference_type(gsl_matrix_uchar_ptr(_matrix, i, j));
#else 
  ...
  //__________________________________________________________________
  inline matrix<unsigned char >::vector_type
  matrix<unsigned char >::operator[](iterator i)
  {
    // Must be implemented here - temporary
    return vector_type(gsl_matrix_uchar_row(_matrix,i));
  }
  //__________________________________________________________________
  inline const matrix<unsigned char >::vector_type
  matrix<unsigned char >::operator[](iterator i) const
  {
    // Must be implemented here - temporary
    return vector_type(gsl_matrix_uchar_row(_matrix,i));
  }
  //__________________________________________________________________
  inline matrix<unsigned char >::value_type*
  matrix<unsigned char >::element(iterator row, iterator column) 
  {
    // Must be implemented here - temporary
    
    return (value_type*)(gsl_matrix_uchar_ptr(_matrix,row, column));
  }
  //__________________________________________________________________
  inline const matrix<unsigned char >::value_type*
  matrix<unsigned char >::element(iterator row, iterator column) const
  {
    // Must be implemented here - temporary
    return (value_type*)(gsl_matrix_uchar_const_ptr(_matrix,row, column));
  }
  //__________________________________________________________________
  inline matrix<unsigned char >::vector_type
  matrix<unsigned char >::row(iterator i)
  {
    return vector_type(gsl_matrix_uchar_row(_matrix,i));
  }
  //__________________________________________________________________
  inline const matrix<unsigned char >::vector_type
  matrix<unsigned char >::row(iterator i) const
  {
    return vector_type(gsl_matrix_uchar_row(_matrix,i));
  }
  //__________________________________________________________________
  inline matrix<unsigned char >::vector_type
  matrix<unsigned char >::column(iterator i)
  {
    return vector_type(gsl_matrix_uchar_column(_matrix,i));
  }
  //__________________________________________________________________
  inline const matrix<unsigned char >::vector_type
  matrix<unsigned char >::column(iterator i) const
  {
    return vector_type(gsl_matrix_uchar_column(_matrix,i));
  }
  //__________________________________________________________________
  inline matrix<unsigned char >::vector_type
  matrix<unsigned char >::diagonal(int i)
  {
    if (i == 0)     
      return vector_type(gsl_matrix_uchar_diagonal(_matrix));
    else if (i < 0) 
      return 
	vector_type(gsl_matrix_uchar_subdiagonal(_matrix,-i));
    return 
      vector_type(gsl_matrix_uchar_superdiagonal(_matrix,i));
  }
  //__________________________________________________________________
  inline const matrix<unsigned char >::vector_type
  matrix<unsigned char >::diagonal(int i) const
  {
    if (i == 0)     
      return vector_type(gsl_matrix_uchar_diagonal(_matrix));
    else if (i < 0) 
  ...
  //__________________________________________________________________
  inline matrix<unsigned char >::vector_type
  matrix<unsigned char >::sub_diagonal(int i)
  {
    return diagonal(-i);
  }
  //__________________________________________________________________
  inline const matrix<unsigned char >::vector_type
  matrix<unsigned char >::sub_diagonal(int i) const
  {
    return diagonal(-i);
  }
  //__________________________________________________________________
  inline matrix<unsigned char >::vector_type
  matrix<unsigned char >::super_diagonal(int i)
  {
    return diagonal(i);
  }
  //__________________________________________________________________
  inline const matrix<unsigned char >::vector_type
  matrix<unsigned char >::super_diagonal(int i) const
  {
    return diagonal(i);
  }
  //__________________________________________________________________
  ...
  matrix<unsigned char >::operator*=(const matrix<value_type>& m)
  {
    gsl_matrix_uchar_mul_elements(_matrix,m._matrix);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<unsigned char >&
  matrix<unsigned char >::operator/=(const matrix<value_type>& m)
  {
    gsl_matrix_uchar_div_elements(_matrix,m._matrix);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<unsigned char >&
  matrix<unsigned char >::operator+=(const matrix<value_type>& m)
  {
    gsl_matrix_uchar_add(_matrix,m._matrix);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<unsigned char >&
  matrix<unsigned char >::operator-=(const matrix<value_type>& m)
  {
    gsl_matrix_uchar_sub(_matrix,m._matrix);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<unsigned char >&
  matrix<unsigned char >::operator+=(const value_type& x)
  {
    gsl_matrix_uchar_add_constant(_matrix,x);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<unsigned char >&
  ...
  matrix<unsigned char >::operator*=(const value_type& x)
  {
    gsl_matrix_uchar_scale(_matrix,x);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<unsigned char >&
  ...
  inline 
  void
  matrix<unsigned char >::conjugate() 
  {
  ...
  //__________________________________________________________________
  inline 
  matrix<unsigned char >::value_type
  matrix<unsigned char >::min() const 
  {
    return gsl_matrix_uchar_min(_matrix);
  }
  //__________________________________________________________________
  inline 
  matrix<unsigned char >::value_type
  matrix<unsigned char >::max() const 
  {
    return gsl_matrix_uchar_max(_matrix);
  }
  //__________________________________________________________________
  inline void
  matrix<unsigned char >::minmax(value_type& min, value_type& max) const 
  {
    return gsl_matrix_uchar_minmax(_matrix, &min, &max);
  }
  //__________________________________________________________________
  inline void
  matrix<unsigned char >::min_index(iterator& row, iterator& column) const 
  {
    gsl_matrix_uchar_min_index(_matrix, &row, &column);
  }
  //__________________________________________________________________
  inline void
  matrix<unsigned char >::max_index(iterator& row, iterator& column) const 
  {
    gsl_matrix_uchar_max_index(_matrix, &row, &column);
  }
  //__________________________________________________________________
  inline void
  matrix<unsigned char >::minmax_index(iterator& min_row, iterator& min_column,
			       iterator& max_row, iterator& max_column) 
    const 
  {
    gsl_matrix_uchar_minmax_index(_matrix, &min_row, &min_column,
					  &max_row, &max_column);
  }
#endif
}
