...
/** @file   vector_double.hh
    @author Christian Holm
    @date   Mon Sep 16 04:41:27 2002
    @brief  Vector specialisation for @c double . */

namespace gslmm 
{
  class permutation;

  /** @brief Vector @c double  specialisation. 
      @ingroup vector 
      @param double   The POD of the specialisation. 
  ...
  */ 
  ...
  public:
    ...
    /** Low level data type  */
    typedef gsl_vector data_type;
    /** Low level view type  */
    typedef gsl_vector_view view_type;
    /** A constant view */
    typedef _gsl_vector_const_view const_view_type;
    /** Trait type */
...
#ifndef GSLMM_vectormatrix_matrix_double
...
namespace gslmm
{
  ...
  inline 
  vector<double >::vector(oper_type m)
    : _vector(0)    
  {
    assert(m.get());
    if (!m.get()) GSL_ERROR_VOID("NULL return passed", GSL_EINVAL);
    _vector = gsl_vector_calloc(m->size());
    m->operator()(*this);
    // delete m;
    // m = 0;
  }
  ...
  //__________________________________________________________________
  inline 
  vector<double >::vector(size_type n, bool zero)
    : _vector(0)    
  {
    if (zero) _vector = gsl_vector_calloc(n);
    else      _vector = gsl_vector_alloc(n);
  }

  //__________________________________________________________________
  inline 
  vector<double >::vector(const vector<value_type>& w, bool really)
    : _vector(0)    
  {
    ...
    if (w.is_owner() || really) {
      _vector = gsl_vector_alloc(w.size()); 
      *this   = w;
    }
    else {
      view_type v = gsl_vector_subvector(w._vector,0,w.size());
      from_view(v);
    }
  }
  ...
  //__________________________________________________________________
  inline 
  vector<double >::vector(const vector<value_type>& w,
			 iterator  offset,
			 size_type n,
			 iterator  stride)
    : _vector(0)    
  {
    view_type v;
    if (stride <= 1) 
      v = gsl_vector_subvector(w._vector, offset, n);
    else 
      v = gsl_vector_subvector_with_stride(w._vector, offset, stride, n);
    from_view(v);
  }

  //__________________________________________________________________
  inline 
  vector<double >::vector(element_type* a, size_type n, iterator  stride)
    : _vector(0)    
  {
    view_type v;
    if (stride <= 1)
      v = gsl_vector_view_array(a, n);
    else
      v = gsl_vector_view_array_with_stride(a,stride,n);
    from_view(v);
  }
  ...
  //__________________________________________________________________
  inline void 
  vector<double >::from_view(const view_type& v) 
  {
  ...
  //__________________________________________________________________
  inline void 
  vector<double >::from_view(const_view_type& v) 
  {
    _vector = static_cast<data_type*>(malloc(sizeof(data_type)));
    _vector->size   = v.vector.size;
    _vector->stride = v.vector.stride;
  ...
  //__________________________________________________________________
  inline const vector<double >::reference_type 
  vector<double >::operator[](iterator  i) 
    const
  {
#if 0
    return reference_type(gsl_vector_const_ptr(_vector,i));
#else
    return reference_type(*gsl_vector_const_ptr(_vector,i));
#endif
  }
  ...
  vector<double >::element(iterator  i) const
  {
#if 0
    return (value_type*)(gsl_vector_const_ptr(_vector,i));
#else
    return const_cast<value_type*>(gsl_vector_const_ptr(_vector,i));
#endif
  }
  ...
  inline 
  vector<double >::value_type* 
  vector<double >::element(iterator  i)
  {
#if 0
    return (value_type*)(gsl_vector_ptr(_vector,i));
#else
    return const_cast<value_type*>(gsl_vector_ptr(_vector,i));
#endif
  }

  //__________________________________________________________________
  inline void vector<double >::zero()
  { 
  ...
  //__________________________________________________________________
  inline void 
  vector<double >::set(value_type x)
  {
    gsl_vector_set_all(_vector,x);
  } 

  //__________________________________________________________________
  inline void 
  vector<double >::basis(iterator  i)
  {
    gsl_vector_set_basis(_vector, i);
  }

  //__________________________________________________________________
  inline vector<double > 
  vector<double >::operator()(iterator  offset, size_type n, iterator  stride)
  {
    return vector<double >(*this, offset, n, stride);
  }
  ...
  //__________________________________________________________________
  inline int vector<double >::swap(vector<value_type>& other)
  {
    return gsl_vector_swap(_vector, other._vector);
  }

  //__________________________________________________________________
  inline int 
  vector<double >::swap(iterator  i, iterator  j)
  {
    return gsl_vector_swap_elements(_vector, i, j);
  }

  //__________________________________________________________________
  inline int vector<double >::reverse()
  {
    return gsl_vector_reverse(_vector);
  }

  //__________________________________________________________________
  inline vector<double >& 
  vector<double >::operator=(const vector<value_type>& w)
  {
    gsl_vector_memcpy(_vector, w._vector); 
    return *this;
  }
  //__________________________________________________________________
  inline vector<double >& 
  vector<double >::operator=(oper_type m)
  {
    assert(m.get());
  ...
  //__________________________________________________________________
  inline bool 
  vector<double >::is_null() const
  {
    return gsl_vector_isnull(_vector) == 1 ? true : false;
  }

#if 0 
  //__________________________________________________________________
  inline matrix<double > 
  vector<double >::outer_product(const vector<value_type> w) const
  {
    matrix<double > m(size(),w.size());
    for (vector<double >::iterator  i = 0; i < size(); i++)
      for (vector<double >::iterator  j = 0; j < w.size(); j++)
	m[i][j] = (*this)[i] * w[j];
    return m;
  } 
#endif
  ...
  //__________________________________________________________________
  inline vector<double >::value_type 
  vector<double >::inner_product(const vector<value_type>& w)  const
  {
    value_type ret;
    for (vector<double >::iterator  i = 0; 
	 i < size() && i < w.size(); i++)
  ...
  {
    return 
      vector<double>(gsl_vector_real(this->_vector));
  }
  ...
  {
    return 
      vector<double>(gsl_vector_const_real(this->_vector));
  }
  ...
  {
    return 
      vector<double>(gsl_vector_imag(this->_vector));
  }
  ...
  {
    return 
      vector<double>(gsl_vector_const_imag(this->_vector));
  }

#define _GSLMM_VECTOR_FOREACH(w,f) \
  do { for (vector<double >::iterator i = this->begin(); \
...
#define _GSLMM_VECTOR_FOREACH(x,f) \
  do { for (vector<double >::iterator i = this->begin(); i != this->end(); i++)\
    this->operator[](i) f x; } while(false) 
  
  //__________________________________________________________________
  inline vector<double >&
  vector<double >::operator+=(const value_type x)
  {
...
#undef _GSLMM_VECTOR_FOREACH
  ...
  vector<double >::operator+=(const vector<value_type>& w)
  {
    gsl_vector_add(_vector, w._vector); return *this;
  }
  ...
  vector<double >::operator-=(const vector<value_type>& w)
  {
    gsl_vector_sub(_vector, w._vector); return *this;
  }
  ...
  vector<double >::operator*=(const vector<value_type>& w)
  {
    gsl_vector_mul(_vector, w._vector); return *this;
  }
  ...
  vector<double >::operator/=(const vector<value_type>& w)
  {
    gsl_vector_div(_vector, w._vector); return *this;
  }

  //__________________________________________________________________
  inline 
  vector<double >& 
  vector<double >::operator+=(const value_type a)
  {
    gsl_vector_add_constant(_vector, a); return *this;
  }
  ...
  vector<double >::operator*=(const value_type a)
  {
    gsl_vector_scale(_vector, a); return *this;
  }
  ...
  vector<double >::operator/=(const value_type a)
  {
    gsl_vector_scale(_vector, 1 / a); return *this;
  }
  ...
  {
    value_type ret = 0;
    for (vector<double >::iterator  i = 0; i < size() && i < w.size(); i++)
      ret += this->operator[](i) * w[i];
    return ret;
  }
  ...
  inline 
  void 
  vector<double >::minmax(value_type& min, value_type& max) const
  {
    return gsl_vector_minmax(_vector, &min, &max);
  }

  //__________________________________________________________________
  inline 
  vector<double >::iterator  
  vector<double >::min_index() const
  {
    return gsl_vector_min_index(_vector);
  }

  //__________________________________________________________________
  inline 
  vector<double >::iterator  
  vector<double >::max_index() const
  {
    return gsl_vector_max_index(_vector);
  }

  //__________________________________________________________________
  inline 
  void 
  vector<double >::minmax_index(iterator& min, iterator& max) const
  {
    return gsl_vector_minmax_index(_vector, &min, &max);
  }
#endif
}
