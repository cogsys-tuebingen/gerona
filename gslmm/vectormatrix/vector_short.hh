...
/** @file   vector_short.hh
    @author Christian Holm
    @date   Mon Sep 16 04:41:27 2002
    @brief  Vector specialisation for @c short . */

namespace gslmm 
{
  class permutation;

  /** @brief Vector @c short  specialisation. 
      @ingroup vector 
      @param short   The POD of the specialisation. 
  ...
  */ 
  ...
  public:
    ...
    /** Low level data type  */
    typedef gsl_vector_short data_type;
    /** Low level view type  */
    typedef gsl_vector_short_view view_type;
    /** A constant view */
    typedef _gsl_vector_short_const_view const_view_type;
    /** Trait type */
...
#ifndef GSLMM_vectormatrix_matrix_short
...
namespace gslmm
{
  ...
  inline 
  vector<short >::vector(oper_type m)
    : _vector(0)    
  {
    assert(m.get());
    if (!m.get()) GSL_ERROR_VOID("NULL return passed", GSL_EINVAL);
    _vector = gsl_vector_short_calloc(m->size());
    m->operator()(*this);
    // delete m;
    // m = 0;
  }
  ...
  //__________________________________________________________________
  inline 
  vector<short >::vector(size_type n, bool zero)
    : _vector(0)    
  {
    if (zero) _vector = gsl_vector_short_calloc(n);
    else      _vector = gsl_vector_short_alloc(n);
  }

  //__________________________________________________________________
  inline 
  vector<short >::vector(const vector<value_type>& w, bool really)
    : _vector(0)    
  {
    ...
    if (w.is_owner() || really) {
      _vector = gsl_vector_short_alloc(w.size()); 
      *this   = w;
    }
    else {
      view_type v = gsl_vector_short_subvector(w._vector,0,w.size());
      from_view(v);
    }
  }
  ...
  //__________________________________________________________________
  inline 
  vector<short >::vector(const vector<value_type>& w,
			 iterator  offset,
			 size_type n,
			 iterator  stride)
    : _vector(0)    
  {
    view_type v;
    if (stride <= 1) 
      v = gsl_vector_short_subvector(w._vector, offset, n);
    else 
      v = gsl_vector_short_subvector_with_stride(w._vector, offset, stride, n);
    from_view(v);
  }

  //__________________________________________________________________
  inline 
  vector<short >::vector(element_type* a, size_type n, iterator  stride)
    : _vector(0)    
  {
    view_type v;
    if (stride <= 1)
      v = gsl_vector_short_view_array(a, n);
    else
      v = gsl_vector_short_view_array_with_stride(a,stride,n);
    from_view(v);
  }
  ...
  vector<short >::~vector()
  {
    gsl_vector_short_free(_vector);
  }

  //__________________________________________________________________
  inline void 
  vector<short >::from_view(const view_type& v) 
  {
  ...
  //__________________________________________________________________
  inline void 
  vector<short >::from_view(const_view_type& v) 
  {
    _vector = static_cast<data_type*>(malloc(sizeof(data_type)));
    _vector->size   = v.vector.size;
    _vector->stride = v.vector.stride;
  ...
  //__________________________________________________________________
  inline const vector<short >::reference_type 
  vector<short >::operator[](iterator  i) 
    const
  {
#if 0
    return reference_type(gsl_vector_short_const_ptr(_vector,i));
#else
    return reference_type(*gsl_vector_short_const_ptr(_vector,i));
#endif
  }
  ...
  vector<short >::element(iterator  i) const
  {
#if 0
    return (value_type*)(gsl_vector_short_const_ptr(_vector,i));
#else
    return const_cast<value_type*>(gsl_vector_short_const_ptr(_vector,i));
#endif
  }
  ...
  inline 
  vector<short >::value_type* 
  vector<short >::element(iterator  i)
  {
#if 0
    return (value_type*)(gsl_vector_short_ptr(_vector,i));
#else
    return const_cast<value_type*>(gsl_vector_short_ptr(_vector,i));
#endif
  }

  //__________________________________________________________________
  inline void vector<short >::zero()
  { 
  ...
  //__________________________________________________________________
  inline void 
  vector<short >::set(value_type x)
  {
    gsl_vector_short_set_all(_vector,x);
  } 

  //__________________________________________________________________
  inline void 
  vector<short >::basis(iterator  i)
  {
    gsl_vector_short_set_basis(_vector, i);
  }

  //__________________________________________________________________
  inline vector<short > 
  vector<short >::operator()(iterator  offset, size_type n, iterator  stride)
  {
    return vector<short >(*this, offset, n, stride);
  }
  ...
  //__________________________________________________________________
  inline int vector<short >::swap(vector<value_type>& other)
  {
    return gsl_vector_short_swap(_vector, other._vector);
  }

  //__________________________________________________________________
  inline int 
  vector<short >::swap(iterator  i, iterator  j)
  {
    return gsl_vector_short_swap_elements(_vector, i, j);
  }

  //__________________________________________________________________
  inline int vector<short >::reverse()
  {
    return gsl_vector_short_reverse(_vector);
  }

  //__________________________________________________________________
  inline vector<short >& 
  vector<short >::operator=(const vector<value_type>& w)
  {
    gsl_vector_short_memcpy(_vector, w._vector); 
    return *this;
  }
  //__________________________________________________________________
  inline vector<short >& 
  vector<short >::operator=(oper_type m)
  {
    assert(m.get());
  ...
  //__________________________________________________________________
  inline bool 
  vector<short >::is_null() const
  {
    return gsl_vector_short_isnull(_vector) == 1 ? true : false;
  }

#if 0 
  //__________________________________________________________________
  inline matrix<short > 
  vector<short >::outer_product(const vector<value_type> w) const
  {
    matrix<short > m(size(),w.size());
    for (vector<short >::iterator  i = 0; i < size(); i++)
      for (vector<short >::iterator  j = 0; j < w.size(); j++)
	m[i][j] = (*this)[i] * w[j];
    return m;
  } 
#endif
  ...
  //__________________________________________________________________
  inline vector<short >::value_type 
  vector<short >::inner_product(const vector<value_type>& w)  const
  {
    value_type ret;
    for (vector<short >::iterator  i = 0; 
	 i < size() && i < w.size(); i++)
  ...
  {
    return 
      vector<short>(gsl_vector_short_real(this->_vector));
  }

  //__________________________________________________________________
  inline const vector<short> 
  vector<short >::real() const
  {
    return 
      vector<short>(gsl_vector_short_const_real(this->_vector));
  }
  ...
  {
    return 
      vector<short>(gsl_vector_short_imag(this->_vector));
  }

  //__________________________________________________________________
  inline const vector<short> 
  vector<short >::imag() const 
  {
    return 
      vector<short>(gsl_vector_short_const_imag(this->_vector));
  }

#define _GSLMM_VECTOR_FOREACH(w,f) \
  do { for (vector<short >::iterator i = this->begin(); \
...
#define _GSLMM_VECTOR_FOREACH(x,f) \
  do { for (vector<short >::iterator i = this->begin(); i != this->end(); i++)\
    this->operator[](i) f x; } while(false) 
  
  //__________________________________________________________________
  inline vector<short >&
  vector<short >::operator+=(const value_type x)
  {
...
#undef _GSLMM_VECTOR_FOREACH
  ...
  vector<short >::operator+=(const vector<value_type>& w)
  {
    gsl_vector_short_add(_vector, w._vector); return *this;
  }
  ...
  vector<short >::operator-=(const vector<value_type>& w)
  {
    gsl_vector_short_sub(_vector, w._vector); return *this;
  }
  ...
  vector<short >::operator*=(const vector<value_type>& w)
  {
    gsl_vector_short_mul(_vector, w._vector); return *this;
  }
  ...
  vector<short >::operator/=(const vector<value_type>& w)
  {
    gsl_vector_short_div(_vector, w._vector); return *this;
  }

  //__________________________________________________________________
  inline 
  vector<short >& 
  vector<short >::operator+=(const value_type a)
  {
    gsl_vector_short_add_constant(_vector, a); return *this;
  }
  ...
  vector<short >::operator*=(const value_type a)
  {
    gsl_vector_short_scale(_vector, a); return *this;
  }
  ...
  vector<short >::operator/=(const value_type a)
  {
    gsl_vector_short_scale(_vector, 1 / a); return *this;
  }
  ...
  {
    value_type ret = 0;
    for (vector<short >::iterator  i = 0; i < size() && i < w.size(); i++)
      ret += this->operator[](i) * w[i];
    return ret;
  }
  ...
  inline 
  vector<short >::value_type 
  vector<short >::min() const
  {
    return gsl_vector_short_min(_vector);
  }

  //__________________________________________________________________
  inline 
  vector<short >::value_type 
  vector<short >::max() const
  {
    return gsl_vector_short_max(_vector);
  }

  //__________________________________________________________________
  inline 
  void 
  vector<short >::minmax(value_type& min, value_type& max) const
  {
    return gsl_vector_short_minmax(_vector, &min, &max);
  }

  //__________________________________________________________________
  inline 
  vector<short >::iterator  
  vector<short >::min_index() const
  {
    return gsl_vector_short_min_index(_vector);
  }

  //__________________________________________________________________
  inline 
  vector<short >::iterator  
  vector<short >::max_index() const
  {
    return gsl_vector_short_max_index(_vector);
  }

  //__________________________________________________________________
  inline 
  void 
  vector<short >::minmax_index(iterator& min, iterator& max) const
  {
    return gsl_vector_short_minmax_index(_vector, &min, &max);
  }
#endif
}
...
#if 0
#if 0
# ifndef GSLMM_blas_blas_short
#  include <gslmm/blas/blas_short.hh>
# endif
#endif
#endif
