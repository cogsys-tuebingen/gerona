...
#define GSLMM_matrix_float
#line 29 "matrix_skel.hh"
/** @file   matrix_float.hh
    @author Christian Holm
    @date   Mon Mar 10 14:50:12 2003
    @brief  @c float  specialisation of matrix class */
...
#endif
#ifndef GSLMM_vectormatrix_vector_float
...
namespace gslmm
{
  /** @brief Matrix @c float  specialisation.
      @ingroup matrix
   */
  template <>
  class matrix<float >
  ...
  public:
    ...
    /** Low level data type */
    typedef gsl_matrix_float data_type;
    /** Low level view type */
    typedef gsl_matrix_float_view view_type;
    ...
    matrix<float > operator()(iterator row, iterator column, 
...
namespace gslmm
{
  ...
  inline matrix<float >::matrix(oper_type p) 
    : _matrix(0)
  {
    assert(p.get());
    if (!p.get()) GSL_ERROR_VOID("NULL return passed", GSL_EINVAL);
    _matrix = gsl_matrix_float_calloc(p->row_size(), p->column_size());
    p->operator()(*this);
    // delete p;
    // p = 0;
  }
  ...
  //__________________________________________________________________
  inline matrix<float >::matrix(size_t rows, size_t columns, bool zero) 
    : _matrix(0)
  {
    if (zero) _matrix = gsl_matrix_float_calloc(rows, columns);
    else      _matrix = gsl_matrix_float_alloc(rows, columns);
  }
  //__________________________________________________________________
  inline matrix<float >::matrix(const matrix<value_type>& m,
				bool trans
#if 0
				, bool conj
#endif
  ...
  {
    if (!trans) {
      _matrix = gsl_matrix_float_alloc(m.row_size(), m.column_size());
      *this = m;
    }
    else {
      _matrix = gsl_matrix_float_alloc(m.column_size(), m.row_size());
      gsl_matrix_float_transpose_memcpy(_matrix,m._matrix);
    }
#if 0
    if (conj) conjugate();
#endif
  ...
  //__________________________________________________________________
  inline 
  matrix<float >::matrix(const matrix<value_type>& m, 
			 iterator row_offset, iterator column_offset, 
			 size_type rows, size_type columns, 
			 size_type) 
    : _matrix(0)
  {
    view_type v = gsl_matrix_float_submatrix(m._matrix,
					    row_offset, 
					    column_offset,
					    rows, columns);
    from_view(v);
  ...
  //__________________________________________________________________
  inline matrix<float >::matrix(element_type* array, 
				size_type rows, size_type columns, 
				size_type tda) 
    : _matrix(0)
  {
    view_type v;
    if (tda <= 0)
      v = gsl_matrix_float_view_array(array, rows, columns);
    else 
      v = gsl_matrix_float_view_array_with_tda(array, rows, columns, tda);
    from_view(v);
  }
  //__________________________________________________________________
  inline 
  matrix<float >::matrix(const vector_type& vec,
			 size_type rows, size_type columns, 
			 size_type tda) 
     : _matrix(0)
 {
    view_type v;
    if (tda <= 0)
      v = gsl_matrix_float_view_vector(vec._vector, rows, columns);
    else 
      v = gsl_matrix_float_view_vector_with_tda(vec._vector, rows, 
							columns, tda);
    from_view(v);
  }
  ...
  //__________________________________________________________________
  inline void matrix<float >::from_view(view_type& v)
  {
  ...
  //__________________________________________________________________
  inline matrix<float >&
  matrix<float >::operator=(const matrix<value_type>& m)
  {
    gsl_matrix_float_memcpy(_matrix,m._matrix);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<float >&
  matrix<float >::operator=(oper_type p)
  {
    assert(p.get());
  ...
  //__________________________________________________________________
  inline void
  matrix<float >::swap(const matrix<value_type>& m)
  {
    gsl_matrix_float_swap(_matrix,m._matrix);
  }
  //__________________________________________________________________
  inline void
  matrix<float >::swap_rows(iterator i, iterator j)
  {
    gsl_matrix_float_swap_rows(_matrix,i, j);
  }
  //__________________________________________________________________
  inline void
  matrix<float >::swap_columns(iterator i, iterator j)
  {
    gsl_matrix_float_swap_columns(_matrix,i, j);
  }
  //__________________________________________________________________
  inline void
  matrix<float >::swap_row_column(iterator i, iterator j)
  {
    gsl_matrix_float_swap_rowcol(_matrix,i, j);
  }
  //__________________________________________________________________
  inline bool
  matrix<float >::is_owner()  const
  {
    return _matrix->owner == 1;
  }
  //__________________________________________________________________
  inline bool
  matrix<float >::is_null()  const 
  {
    return gsl_matrix_float_isnull(_matrix) == 1;
  }
  //__________________________________________________________________
  inline void
  matrix<float >::zero() 
  ...
  //__________________________________________________________________
  inline void
  matrix<float >::set(value_type x) 
  {
    gsl_matrix_float_set_all(_matrix, x);
  }
  //__________________________________________________________________
  inline void
  matrix<float >::identity()
  {
    gsl_matrix_float_set_identity(_matrix);
  }
  //__________________________________________________________________
  inline void
  matrix<float >::transpose()
  {
    gsl_matrix_float_transpose(_matrix);
  }
  //__________________________________________________________________
  inline matrix<float >
  matrix<float >::operator()(iterator i, iterator j, size_type n1, 
				   size_type n2) const
  {
    return matrix<float >(*this, i, j, n1, n2);
  }
  //__________________________________________________________________
  inline 
  const matrix<float >::reference_type
  matrix<float >::operator()(iterator i, iterator j) const 
  {
#if 0
    return reference_type(gsl_matrix_float_ptr(_matrix, i, j));
#else 
    return reference_type(*(gsl_matrix_float_ptr(_matrix, i, j)));
#endif
  }
  //__________________________________________________________________
  inline 
  matrix<float >::reference_type
  matrix<float >::operator()(iterator i, iterator j) 
  {
#if 0
    return reference_type(gsl_matrix_float_ptr(_matrix, i, j));
#else 
  ...
  //__________________________________________________________________
  inline matrix<float >::vector_type
  matrix<float >::operator[](iterator i)
  {
    // Must be implemented here - temporary
    return vector_type(gsl_matrix_float_row(_matrix,i));
  }
  //__________________________________________________________________
  inline const matrix<float >::vector_type
  matrix<float >::operator[](iterator i) const
  {
    // Must be implemented here - temporary
    return vector_type(gsl_matrix_float_row(_matrix,i));
  }
  //__________________________________________________________________
  inline matrix<float >::value_type*
  matrix<float >::element(iterator row, iterator column) 
  {
    // Must be implemented here - temporary
    
    return (value_type*)(gsl_matrix_float_ptr(_matrix,row, column));
  }
  //__________________________________________________________________
  inline const matrix<float >::value_type*
  matrix<float >::element(iterator row, iterator column) const
  {
    // Must be implemented here - temporary
    return (value_type*)(gsl_matrix_float_const_ptr(_matrix,row, column));
  }
  //__________________________________________________________________
  inline matrix<float >::vector_type
  matrix<float >::row(iterator i)
  {
    return vector_type(gsl_matrix_float_row(_matrix,i));
  }
  //__________________________________________________________________
  inline const matrix<float >::vector_type
  matrix<float >::row(iterator i) const
  {
    return vector_type(gsl_matrix_float_row(_matrix,i));
  }
  //__________________________________________________________________
  inline matrix<float >::vector_type
  matrix<float >::column(iterator i)
  {
    return vector_type(gsl_matrix_float_column(_matrix,i));
  }
  //__________________________________________________________________
  inline const matrix<float >::vector_type
  matrix<float >::column(iterator i) const
  {
    return vector_type(gsl_matrix_float_column(_matrix,i));
  }
  //__________________________________________________________________
  inline matrix<float >::vector_type
  matrix<float >::diagonal(int i)
  {
    if (i == 0)     
      return vector_type(gsl_matrix_float_diagonal(_matrix));
    else if (i < 0) 
      return 
	vector_type(gsl_matrix_float_subdiagonal(_matrix,-i));
    return 
      vector_type(gsl_matrix_float_superdiagonal(_matrix,i));
  }
  //__________________________________________________________________
  inline const matrix<float >::vector_type
  matrix<float >::diagonal(int i) const
  {
    if (i == 0)     
      return vector_type(gsl_matrix_float_diagonal(_matrix));
    else if (i < 0) 
  ...
  //__________________________________________________________________
  inline matrix<float >::vector_type
  matrix<float >::sub_diagonal(int i)
  {
    return diagonal(-i);
  }
  //__________________________________________________________________
  inline const matrix<float >::vector_type
  matrix<float >::sub_diagonal(int i) const
  {
    return diagonal(-i);
  }
  //__________________________________________________________________
  inline matrix<float >::vector_type
  matrix<float >::super_diagonal(int i)
  {
    return diagonal(i);
  }
  //__________________________________________________________________
  inline const matrix<float >::vector_type
  matrix<float >::super_diagonal(int i) const
  {
    return diagonal(i);
  }
  //__________________________________________________________________
  ...
  matrix<float >::operator*=(const matrix<value_type>& m)
  {
    gsl_matrix_float_mul_elements(_matrix,m._matrix);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<float >&
  matrix<float >::operator/=(const matrix<value_type>& m)
  {
    gsl_matrix_float_div_elements(_matrix,m._matrix);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<float >&
  matrix<float >::operator+=(const matrix<value_type>& m)
  {
    gsl_matrix_float_add(_matrix,m._matrix);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<float >&
  matrix<float >::operator-=(const matrix<value_type>& m)
  {
    gsl_matrix_float_sub(_matrix,m._matrix);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<float >&
  matrix<float >::operator+=(const value_type& x)
  {
    gsl_matrix_float_add_constant(_matrix,x);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<float >&
  ...
  matrix<float >::operator*=(const value_type& x)
  {
    gsl_matrix_float_scale(_matrix,x);
    return *this;
  }
  //__________________________________________________________________
  inline matrix<float >&
  ...
  //__________________________________________________________________
  inline 
  matrix<float >::value_type
  matrix<float >::min() const 
  {
    return gsl_matrix_float_min(_matrix);
  }
  //__________________________________________________________________
  inline 
  matrix<float >::value_type
  matrix<float >::max() const 
  {
    return gsl_matrix_float_max(_matrix);
  }
  //__________________________________________________________________
  inline void
  matrix<float >::minmax(value_type& min, value_type& max) const 
  {
    return gsl_matrix_float_minmax(_matrix, &min, &max);
  }
  //__________________________________________________________________
  inline void
  matrix<float >::min_index(iterator& row, iterator& column) const 
  {
    gsl_matrix_float_min_index(_matrix, &row, &column);
  }
  //__________________________________________________________________
  inline void
  matrix<float >::max_index(iterator& row, iterator& column) const 
  {
    gsl_matrix_float_max_index(_matrix, &row, &column);
  }
  //__________________________________________________________________
  inline void
  matrix<float >::minmax_index(iterator& min_row, iterator& min_column,
			       iterator& max_row, iterator& max_column) 
    const 
  {
    gsl_matrix_float_minmax_index(_matrix, &min_row, &min_column,
					  &max_row, &max_column);
  }
#endif
}
