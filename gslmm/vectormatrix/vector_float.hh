...
/** @file   vector_float.hh
    @author Christian Holm
    @date   Mon Sep 16 04:41:27 2002
    @brief  Vector specialisation for @c float . */

namespace gslmm 
{
  class permutation;

  /** @brief Vector @c float  specialisation. 
      @ingroup vector 
      @param float   The POD of the specialisation. 
  ...
  */ 
  ...
  public:
    ...
    /** Low level data type  */
    typedef gsl_vector_float data_type;
    /** Low level view type  */
    typedef gsl_vector_float_view view_type;
    /** A constant view */
    typedef _gsl_vector_float_const_view const_view_type;
    /** Trait type */
...
#ifndef GSLMM_vectormatrix_matrix_float
...
namespace gslmm
{
  ...
  inline 
  vector<float >::vector(oper_type m)
    : _vector(0)    
  {
    assert(m.get());
    if (!m.get()) GSL_ERROR_VOID("NULL return passed", GSL_EINVAL);
    _vector = gsl_vector_float_calloc(m->size());
    m->operator()(*this);
    // delete m;
    // m = 0;
  }
  ...
  //__________________________________________________________________
  inline 
  vector<float >::vector(size_type n, bool zero)
    : _vector(0)    
  {
    if (zero) _vector = gsl_vector_float_calloc(n);
    else      _vector = gsl_vector_float_alloc(n);
  }

  //__________________________________________________________________
  inline 
  vector<float >::vector(const vector<value_type>& w, bool really)
    : _vector(0)    
  {
    ...
    if (w.is_owner() || really) {
      _vector = gsl_vector_float_alloc(w.size()); 
      *this   = w;
    }
    else {
      view_type v = gsl_vector_float_subvector(w._vector,0,w.size());
      from_view(v);
    }
  }
  ...
  //__________________________________________________________________
  inline 
  vector<float >::vector(const vector<value_type>& w,
			 iterator  offset,
			 size_type n,
			 iterator  stride)
    : _vector(0)    
  {
    view_type v;
    if (stride <= 1) 
      v = gsl_vector_float_subvector(w._vector, offset, n);
    else 
      v = gsl_vector_float_subvector_with_stride(w._vector, offset, stride, n);
    from_view(v);
  }

  //__________________________________________________________________
  inline 
  vector<float >::vector(element_type* a, size_type n, iterator  stride)
    : _vector(0)    
  {
    view_type v;
    if (stride <= 1)
      v = gsl_vector_float_view_array(a, n);
    else
      v = gsl_vector_float_view_array_with_stride(a,stride,n);
    from_view(v);
  }
  ...
  vector<float >::~vector()
  {
    gsl_vector_float_free(_vector);
  }

  //__________________________________________________________________
  inline void 
  vector<float >::from_view(const view_type& v) 
  {
  ...
  //__________________________________________________________________
  inline void 
  vector<float >::from_view(const_view_type& v) 
  {
    _vector = static_cast<data_type*>(malloc(sizeof(data_type)));
    _vector->size   = v.vector.size;
    _vector->stride = v.vector.stride;
  ...
  //__________________________________________________________________
  inline const vector<float >::reference_type 
  vector<float >::operator[](iterator  i) 
    const
  {
#if 0
    return reference_type(gsl_vector_float_const_ptr(_vector,i));
#else
    return reference_type(*gsl_vector_float_const_ptr(_vector,i));
#endif
  }
  ...
  vector<float >::element(iterator  i) const
  {
#if 0
    return (value_type*)(gsl_vector_float_const_ptr(_vector,i));
#else
    return const_cast<value_type*>(gsl_vector_float_const_ptr(_vector,i));
#endif
  }
  ...
  inline 
  vector<float >::value_type* 
  vector<float >::element(iterator  i)
  {
#if 0
    return (value_type*)(gsl_vector_float_ptr(_vector,i));
#else
    return const_cast<value_type*>(gsl_vector_float_ptr(_vector,i));
#endif
  }

  //__________________________________________________________________
  inline void vector<float >::zero()
  { 
  ...
  //__________________________________________________________________
  inline void 
  vector<float >::set(value_type x)
  {
    gsl_vector_float_set_all(_vector,x);
  } 

  //__________________________________________________________________
  inline void 
  vector<float >::basis(iterator  i)
  {
    gsl_vector_float_set_basis(_vector, i);
  }

  //__________________________________________________________________
  inline vector<float > 
  vector<float >::operator()(iterator  offset, size_type n, iterator  stride)
  {
    return vector<float >(*this, offset, n, stride);
  }
  ...
  //__________________________________________________________________
  inline int vector<float >::swap(vector<value_type>& other)
  {
    return gsl_vector_float_swap(_vector, other._vector);
  }

  //__________________________________________________________________
  inline int 
  vector<float >::swap(iterator  i, iterator  j)
  {
    return gsl_vector_float_swap_elements(_vector, i, j);
  }

  //__________________________________________________________________
  inline int vector<float >::reverse()
  {
    return gsl_vector_float_reverse(_vector);
  }

  //__________________________________________________________________
  inline vector<float >& 
  vector<float >::operator=(const vector<value_type>& w)
  {
    gsl_vector_float_memcpy(_vector, w._vector); 
    return *this;
  }
  //__________________________________________________________________
  inline vector<float >& 
  vector<float >::operator=(oper_type m)
  {
    assert(m.get());
  ...
  //__________________________________________________________________
  inline bool 
  vector<float >::is_null() const
  {
    return gsl_vector_float_isnull(_vector) == 1 ? true : false;
  }

#if 0 
  //__________________________________________________________________
  inline matrix<float > 
  vector<float >::outer_product(const vector<value_type> w) const
  {
    matrix<float > m(size(),w.size());
    for (vector<float >::iterator  i = 0; i < size(); i++)
      for (vector<float >::iterator  j = 0; j < w.size(); j++)
	m[i][j] = (*this)[i] * w[j];
    return m;
  } 
#endif
  ...
  //__________________________________________________________________
  inline vector<float >::value_type 
  vector<float >::inner_product(const vector<value_type>& w)  const
  {
    value_type ret;
    for (vector<float >::iterator  i = 0; 
	 i < size() && i < w.size(); i++)
  ...
  {
    return 
      vector<float>(gsl_vector_float_real(this->_vector));
  }

  //__________________________________________________________________
  inline const vector<float> 
  vector<float >::real() const
  {
    return 
      vector<float>(gsl_vector_float_const_real(this->_vector));
  }
  ...
  {
    return 
      vector<float>(gsl_vector_float_imag(this->_vector));
  }

  //__________________________________________________________________
  inline const vector<float> 
  vector<float >::imag() const 
  {
    return 
      vector<float>(gsl_vector_float_const_imag(this->_vector));
  }

#define _GSLMM_VECTOR_FOREACH(w,f) \
  do { for (vector<float >::iterator i = this->begin(); \
...
#define _GSLMM_VECTOR_FOREACH(x,f) \
  do { for (vector<float >::iterator i = this->begin(); i != this->end(); i++)\
    this->operator[](i) f x; } while(false) 
  
  //__________________________________________________________________
  inline vector<float >&
  vector<float >::operator+=(const value_type x)
  {
...
#undef _GSLMM_VECTOR_FOREACH
  ...
  vector<float >::operator+=(const vector<value_type>& w)
  {
    gsl_vector_float_add(_vector, w._vector); return *this;
  }
  ...
  vector<float >::operator-=(const vector<value_type>& w)
  {
    gsl_vector_float_sub(_vector, w._vector); return *this;
  }
  ...
  vector<float >::operator*=(const vector<value_type>& w)
  {
    gsl_vector_float_mul(_vector, w._vector); return *this;
  }
  ...
  vector<float >::operator/=(const vector<value_type>& w)
  {
    gsl_vector_float_div(_vector, w._vector); return *this;
  }

  //__________________________________________________________________
  inline 
  vector<float >& 
  vector<float >::operator+=(const value_type a)
  {
    gsl_vector_float_add_constant(_vector, a); return *this;
  }
  ...
  vector<float >::operator*=(const value_type a)
  {
    gsl_vector_float_scale(_vector, a); return *this;
  }
  ...
  vector<float >::operator/=(const value_type a)
  {
    gsl_vector_float_scale(_vector, 1 / a); return *this;
  }
  ...
  {
    value_type ret = 0;
    for (vector<float >::iterator  i = 0; i < size() && i < w.size(); i++)
      ret += this->operator[](i) * w[i];
    return ret;
  }
  ...
  inline 
  vector<float >::value_type 
  vector<float >::min() const
  {
    return gsl_vector_float_min(_vector);
  }

  //__________________________________________________________________
  inline 
  vector<float >::value_type 
  vector<float >::max() const
  {
    return gsl_vector_float_max(_vector);
  }

  //__________________________________________________________________
  inline 
  void 
  vector<float >::minmax(value_type& min, value_type& max) const
  {
    return gsl_vector_float_minmax(_vector, &min, &max);
  }

  //__________________________________________________________________
  inline 
  vector<float >::iterator  
  vector<float >::min_index() const
  {
    return gsl_vector_float_min_index(_vector);
  }

  //__________________________________________________________________
  inline 
  vector<float >::iterator  
  vector<float >::max_index() const
  {
    return gsl_vector_float_max_index(_vector);
  }

  //__________________________________________________________________
  inline 
  void 
  vector<float >::minmax_index(iterator& min, iterator& max) const
  {
    return gsl_vector_float_minmax_index(_vector, &min, &max);
  }
#endif
}
